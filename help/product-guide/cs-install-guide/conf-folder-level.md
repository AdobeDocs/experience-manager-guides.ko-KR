...
대상 ![AEM cloud ](assets/aem-cloud-icon.svg) Experience Manager 가이드 as a Cloud Service .

AI 기반 스마트 제안을 구성하고 작성자가 기존 콘텐츠를 재사용하고 올바르고 일관된 콘텐츠 참조를 쉽게 만들 수 있도록 지원할 수 있습니다. 다음 **AI 구성** 탭에서는 웹 편집기에서 스마트 제안 패널의 설정을 제어할 수 있습니다.

글로벌 또는 폴더 수준 프로필에서 표준 AI 구성을 구성하려면 다음 단계를 수행하십시오.
1. 폴더 수준 프로필에 대한 관리자 권한이 있는 관리자 또는 사용자로 Adobe Experience Manager에 로그인합니다.
1. 다음 항목 선택 **Adobe Experience Manager** 맨 위에 연결한 다음 **도구**.
1. 선택 **안내서** 도구 목록에서 를 선택하고 **폴더 프로필** 타일.
1. 구성할 프로필 타일을 선택합니다.
...
1. 관리자는 다음 설정을 구성할 수 있습니다.
   ...
   **최소 문자**: 작성자가 제안서를 가져오기 위해 입력해야 하는 최소 문자 수를 입력합니다. 예를 들어 이 숫자가 7이면 작성자는 최소 7자를 추가하여 스마트 제안을 확인해야 합니다.

   **최대 제안**: 작성자가 콘텐츠를 작성하는 동안 얻을 수 있는 최대 제안 수를 입력합니다. 예를 들어 이 숫자가 5이면 작성자는 5개 이하의 스마트 제안을 볼 수 있습니다.

   **파일 및 폴더**: 스마트 제안을 표시할 파일 또는 폴더를 선택합니다. *콘텐츠의 일관성을 위해 목록에 있는 두 항목 사이에 공통 파일이 있는 것은 아닙니다*. 파일과 폴더를 선택하면 해당 파일과 폴더가 나열됩니다.
...
보기 및 추가 방법에 대해 자세히 알아보기 [AI 기반 스마트 제안](../user-guide/authoring-ai-based-smart-suggestions.md) 을 클릭하여 웹 편집기에서 작성하는 동안 컨텐츠 참조를 추가합니다.
