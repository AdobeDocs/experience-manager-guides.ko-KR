...
이 예제에서는 &quot;TEST EXTENSION&quot;이라는 다른 `tab`과(와) 레이블이 있는 해당 `tab panel`을(를) 갖도록 `right tab panel`을(를) 사용자 지정했습니다. `New Tab Panel`
...
6. [수정 대화 상자를 포함한 수락](./examples/review_app_examples/accept_with_modification_dialog.ts)
