...
| **언어** | 출력을 번역할 언어를 선택하십시오. 루트 맵의 `xml:lang` 특성과 동일한 언어로 출력을 게시하려면 언어를 명시적으로 선택하는 대신 **맵 언어 사용** 옵션을 선택하십시오. <br> 맵에 정의된 `xml:lang`이(가) 없으면 기본적으로 출력은 영어(en_US)로 설정됩니다. 이렇게 하면 부모 맵에 `xml:lang` 특성이 이미 설정되어 있으므로 각 언어에 대해 별도의 출력 사전 설정이 필요하지 않을 때 도움이 됩니다. 이 설정이 다른 유형의 콘텐츠에 어떻게 영향을 미치는지 이해하려면 [DITA 콘텐츠와 출력 템플릿 변수의 언어 해상도](../native-pdf/native-pdf-language-variables.md#language-resolution-for-dita-content-vs-output-template-variables)를 확인하십시오. |
| **DITA-OT 명령줄 인수** | **DITA-OT 사전 처리 사용**&#x200B;을 활성화하면 **DITA-OT 명령줄 인수** 필드를 사용할 수 있습니다. 여기서 출력을 생성하는 동안 DITA-OT에서 처리할 추가 인수를 지정할 수 있습니다. DITA-OT에서 지원되는 명령줄 인수에 대한 자세한 내용은 [DITA-OT 설명서](https://www.dita-ot.org/)를 참조하세요.<br>**참고:** <br> DITA 관계 테이블(`<reltable>`)에 정의된 관련 링크는 기본적으로 기본 PDF 출력에 포함되지 않습니다. 기본 PDF 출력에 관련 링크를 포함하려면 `-Dargs.rellinks=nofamily` DITA-OT 인수를 사용하십시오. <br> 중첩된 맵의 경우 맵 참조에 설정된 `toc="no"` 특성은 기본적으로 TOC에서 자식 주제를 제외하지 않습니다. `-Dpreprocess.move-meta-entries.skip=false` DITA-OT 인수를 사용하여 자식 주제가 이러한 맵의 목차에서 제외되도록 합니다. |
| **사후 생성 워크플로** | AEM에 구성된 모든 워크플로가 포함된 드롭다운 목록을 표시하려면 선택하십시오. PDF 생성 워크플로우가 완료된 후 실행할 워크플로우를 선택할 수 있습니다. |

>[!NOTE]
>
>- DITA 관계 테이블(`<reltable>`)에 정의된 관련 링크는 기본적으로 기본 PDF 출력에 포함되지 않습니다. 이 필드를 사용하여 `-Dargs.rellinks=nofamily` DITA-OT 인수를 전달하고 출력에 관련 링크를 포함하십시오.
>
...
| **확대/축소** | PDF이 열릴 때 표시되는 방식을 보여 주는 페이지 보기 크기를 조정하려면 선택합니다.  <br><ul><li> **기본값** 사용자의 컴퓨터 <br>에서 PDF 뷰어의 기본 설정에 따라 표시됩니다. <li> **100%** 페이지를 실제 크기로 표시합니다.     <br> <li> **페이지 맞춤** 페이지 너비와 높이를 문서 창에 맞춥니다.   .<br> <li> **페이지 너비 맞추기** 페이지 너비를 문서 창의 너비로 채웁니다.  <br> <li> **페이지 높이 맞추기** 페이지 높이를 문서 창의 높이로 채웁니다. </ul> |
...
인쇄 프로덕션 설정을 구성하여 프린터 표시를 할당하고, 페이지 상자를 정의하고, PDF 출력에 대한 색상 및 ICC 프로파일 옵션을 설정합니다. 인쇄 탭은 **프린터 표시**, **페이지 상자**, **색상 및 ICC** 순서로 세 개의 섹션으로 구성되어 있습니다.

## 프린터 표시

인쇄 제작을 위해 문서를 준비할 때 인쇄 중에 정렬, 트리밍 및 색상 선택을 올바르게 하기 위해 페이지 경계에 프린터 표시가 추가됩니다. 프린터 표시를 선택하면 페이지 경계가 해당 표시에 맞게 확장되고 확장된 영역은 인쇄 후 트림됩니다.

다음 설정을 사용하여 PDF 출력에 대한 프린터 표시를 구성합니다.

| 설정 | 설명 |
| --- | --- |
| **모든 프린터 표시** | 모든 프린터 표시 설정을 활성화하거나 비활성화하려면 선택하십시오. **재단 표시**, **재단 표시**, **등록 표시** 및 **색상줄**. **모든 프린터 표시**&#x200B;를 선택한 동안 개별 표시를 활성화하거나 비활성화하면 이 설정이 자동으로 꺼집니다. 이 설정을 선택하지 않으면 개별 표시를 활성화하거나 비활성화할 수 있습니다. |
| **재단선** | 인쇄 후 용지를 트리밍해야 하는 위치를 나타내기 위해 트리밍 영역의 각 모서리에 표시를 하려면 선택합니다. |
| **도련 표시** | 확장된 이미지의 트림 영역을 나타내기 위해 도련 상자의 각 모서리에 표시를 배치하려면 선택합니다. |
| **등록 표시** | 색상 문서에서 다른 분판을 정렬하기 위해 자르기 영역 외부에 표시를 하려면 선택합니다. |
| **색상 막대** | 인쇄 시 색상 일관성을 유지하고 잉크 밀도를 조정하기 위해 트림 영역 외부에 색상 스트립을 추가하려면 선택합니다. |
| **선 너비** | 선택한 프린터 표시를 그리는 데 사용되는 선의 너비를 지정합니다. 이 값은 0.25pt씩 단계적으로 증가합니다. |
| **선 색** | 선택한 프린터 표시를 그리는 데 사용되는 선의 색상을 지정합니다. |

## 페이지 상자

이 섹션을 사용하여 PDF 출력에 대한 전체 페이지 크기와 도련 영역을 정의합니다.

다음 설정을 사용하여 페이지 상자를 구성합니다.

| 설정 | 설명 |
| --- | --- |
| **미디어 상자 크기** | 프린터 표시에서 차지하는 확장 영역을 포함하여 전체 페이지 크기를 지정합니다. 드롭다운 목록을 사용하여 PDF 출력에 대한 페이지 크기를 선택하거나 사용자 지정 크기를 만듭니다. |
| **도련 상자 너비** | 재단 물림 상자 너비를 지정합니다. 재단 물림 상자 너비에는 재단 물림 표시와 재단 물림 선을 지나 연장되는 이미지가 있습니다. |

## 색상 및 ICC

이 섹션을 사용하여 인쇄할 색상 공간을 선택하고 선택적으로 ICC 프로파일을 사용하여 여러 장치의 색상 정확도를 관리합니다. CMYK 색상 공간을 사용하는 경우 색상 관리가 필요한 PDF 적합성에 대해 ICC 색상 프로파일이 필요합니다.

>[!NOTE]
>
> 향상된 ICC 색상 프로파일 처리를 위해 기본 PDF 엔진 v1의 알려진 ICC 색상 프로파일 문제에 대한 수정 사항이 포함된 [기본 PDF 엔진 v2](../native-pdf/new-pdf-engine.md)을 사용하고 있는지 확인하십시오.

색상 공간 및 ICC 프로파일을 구성하려면 다음 설정을 사용합니다.

| 설정 | 설명 |
| --- | --- |
| **색상 공간** | PDF 문서에 대해 **RGB**&#x200B;과(와) **CMYK** 색상 공간 중에서 선택하십시오. 디지털 디스플레이에 사용되는 PDF의 경우 **RGB**&#x200B;을(를) 선택하고 실제 인쇄의 경우 **CMYK**&#x200B;을(를) 선택하십시오. |
| **색상 변환** | 문서에서 정의된 색상을 위에서 선택한 색상 공간으로 변환할 수 있습니다. 이 옵션은 RGB 및 CMYK 색상 공간에 대해 기본적으로 활성화되어 있습니다. 활성화하면 **렌더링 의도** 필드도 사용할 수 있습니다. |
| **렌더링 의도** | **색상 변환** 옵션이 활성화된 경우에만 사용할 수 있습니다. 색상을 대상 색상 공간으로 변환할 때 사용할 렌더링 의도를 선택하십시오.<br>- **기본값**: 기본 렌더링 동작을 사용합니다.<br>- **지각적**: 전체 시각적 모양을 유지하기 위해 색상을 조정합니다.<br>- **상대 색도계**: 가능한 경우 색상 정확도를 유지하면서 대상 색상 공간을 기준으로 색상을 조정합니다.<br> **참고**: Adobe Experience Manager Guides On-Premise 설정에서 이 기능을 사용할 때 기본 네이티브 PDF 엔진에서 지정된 렌더링 의도를 적용할 수 있는 Java 20 이상을 사용할 수 있는지 확인하십시오. |
| **식별자 이름** | 출력 식별자 이름을 입력합니다. |
| **프로필 찾아보기** | AEM 서버에서 ICC 프로필 파일(`.icc` 또는 `.icm`)을 찾습니다. **프로필에 URL 사용**&#x200B;을 사용하도록 설정하면 이 필드를 사용할 수 없습니다. |
| **프로필에 URL 사용** | 로컬 파일 대신 온라인으로 호스팅된 ICC 프로필을 사용하려면 이 옵션을 활성화합니다. |
| **프로필 URL** | **프로필에 URL 사용**&#x200B;을 사용하도록 설정한 경우에만 사용할 수 있습니다. ICC 프로파일이 호스팅되는 URL을 입력합니다. |
