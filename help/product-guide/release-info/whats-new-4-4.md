...
![버전 기록 대화 상자](assets/version-history-dialog-web-editor.png){width="800"}
...
![새 기준선 만들기](assets/dynamic-baseline-4-4.png) {width="300"}
*웹 편집기에서 기준선을 만듭니다.*
...
![번역 패널](assets/translation-languages-4.4.png){width="300"}
...
![삽입 대화 상자](assets/insert-element.png){width="300"}
...
![번역 패널](assets/context-menu-split-list.png){width="300"}
...
Experience Manager Guides을 사용하면 관리자가 AEM 사이트 출력의 URL을 구성할 수 있습니다. 파일 이름이 존재하지 않거나 모든 특수 문자를 포함하는 경우 AEM 사이트 출력의 URL에서 구분 기호로 대체하도록 구성할 수 있습니다. 첫 번째 하위 항목의 이름으로 바꿀 수도 있습니다. [문서 제목을 사용하도록 AEM 사이트 출력의 URL을 구성하는 방법](../cs-install-guide/conf-output-generation.md#configure-the-url-of-the-aem-site-output-to-use-the-document-title)을 알아봅니다.


## 여러 출력 사전 설정을 동시에 게시

Experience Manager에서는 항목에 적용된 레이블에 따라 자동으로 항목을 선택하여 기준선을 만드는 기능을 제공합니다. 이제 동일한 DITA 맵의 자동 기준선을 사용하여 여러 출력 사전 설정을 원활하게 게시할 수도 있습니다. 한 번에 하나의 사전 설정만 게시할 필요는 없지만 여러 출력 사전 설정을 동시에 쉽게 게시할 수 있습니다.

[웹 편집기에서 기준선을 만들고 관리하는 방법](../user-guide/web-editor-baseline.md)에 대해 자세히 알아보세요.

## 기본 PDF 개선 사항

4.4.0 릴리스에서는 다음과 같은 네이티브 PDF 개선이 수행되었습니다.
...
자세한 내용은 [PDF 출력 게시](../web-editor/native-pdf-web-editor.md)에서 **고급** 설정을 확인하세요.
...
자세한 내용은 [PDF 출력 게시](../web-editor/native-pdf-web-editor.md)에서 **메타데이터** 탭을 참조하십시오.
