...
description: Adobe Experience Manager Guides as a Cloud Service의 2023년 12월 릴리스의 새로운 기능과 향상된 기능을 알아보십시오.
feature: What's New
role: Leader
...
# Adobe Experience Manager Guides as a Cloud Service 2023년 12월 릴리스의 새로운 기능
...
![네이티브 pdf 변수](assets/add-variable-default.png){width="800"}
...
PDF 출력에서 [변수를 사용하는 방법](../native-pdf/native-pdf-variables.md)에 대해 자세히 알아보세요.
...
![특성 패널](assets/attributes-multiple-properties.png){width="300"}
...
![파일 속성](assets/file-properties-general.png){width="300"}
...
![서비스 지금 기술 자료 사전 설정](assets/knowledgebase--output-preset.png){width="300"}
...
![맵 컬렉션 대시보드](assets/map-collection-dashboard.png){width="800"}
...
맵 보기의 ![키](assets/view-key-title-map-view.png) {width="300"}
...
![새 기준선 만들기](assets/create-new-baseline.png) {width="300"}
*웹 편집기에서 기준선을 만듭니다.*

또한 레이블을 기반으로 기준선을 복제할 수도 있습니다. 참조 버전은 복제하는 동안 지정된 레이블(있는 경우)을 기반으로 선택되거나, 복제된 기준선에서 버전을 선택합니다.


![기준선 ](assets/duplicate-baseline.png) {width="300"}을(를) 복제합니다.

*레이블을 기반으로 기준선을 복제하거나 정확한 복사본을 만듭니다.*

[웹 편집기에서 기준선을 만들고 관리하는 방법](../user-guide/web-editor-baseline.md)에 대해 자세히 알아보세요.

## AEM 사이트 출력에서 교차 맵 링크 해결

AEM 사이트 출력에 렌더링되는 교차 맵 링크(범위 피어가 있는 XREF)는 이제 생성된 맵에 대해 설정된 게시 컨텍스트의 파일 제목에 따라 확인됩니다.


## 문서 제목을 사용하도록 AEM 사이트 출력의 URL을 구성합니다

Experience Manager Guides을 사용하면 관리자가 AEM 사이트 출력의 URL을 구성할 수 있습니다. 파일 이름이 존재하지 않거나 모든 특수 문자를 포함하는 경우 AEM 사이트 출력의 URL에서 구분 기호로 대체하도록 구성할 수 있습니다. 첫 번째 하위 주제의 이름으로 바꿀 수도 있습니다. [문서 제목을 사용하도록 AEM 사이트 출력의 URL을 구성하는 방법](../cs-install-guide/conf-output-generation.md#configure-the-url-of-the-aem-site-output-to-use-the-document-title)을 알아봅니다.
