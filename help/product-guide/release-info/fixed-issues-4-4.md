...
- 편집기에서 맵 다운로드 기능이 다운로드할 준비가 되면 팝업 알림을 트리거하지 못합니다. (14626)
- 편집기에서 맵 다운로드 기능이 기준선이 있는 맵을 다운로드하지 못합니다. (14622)
- Experience Manager Guides에서 잘못된 DTD 오류가 발생했습니다. (14482)
- 편집기 탭의 제목은 점(.) 다음에 잘립니다. 문자. (14372)
...
- **레이아웃** 보기에서 특수 스키마를 사용하여 새 참조 또는 주제를 추가할 때 편집기에서 응용 프로그램 오류가 발생합니다. (14094)
...
- 편집기의 **즐겨찾기** 컬렉션을 로드하지 못했습니다. (13495)
...
- Zip 파일은 편집기에서 인식되지 않으며 드래그하여 놓을 수 없습니다. (12709)
...
- 사용자가 **사용자 환경 설정**&#x200B;에서 명시적으로 설정하지 않았더라도 Rootmap 설정은 편집기에서 유지됩니다. (11551)
- 일부 특성이 적용된 콘텐츠가 **작성자** 또는 **미리 보기** 모드에서 강조 표시되지 않습니다. (11063)
- 편집기에서 `varname` 태그를 삽입할 때 **키 참조** 창이 열립니다. (10940)
- 저장소에서 용어집 주제를 용어집 맵으로 드래그하면 `topicref`이(가) 만들어집니다. (10767)
- Google Chrome 및 Microsoft Edge 브라우저에서 XML 편집기의 미리 보기 창이 잘립니다. (10755)
- 편집기에는 가능한 상위 요소 내에 요소를 래핑하는 기능이 없습니다. (8791)
- Adobe Experience Manager Guides 버전 4.3.1을 다시 설치하면 편집기가 제거됩니다. (14519)
...
- **AEM 사이트** 사전 설정에서 편집기의 `/content` 계층 외부에 있는 템플릿을 선택할 수 없습니다. (14260)
...
- AEM 사이트 게시에서 범위 피어 링크를 사용하는 큰 DITA 맵에 대한 데이터 저장소를 커밋할 때 문제가 발생합니다. (13530)
- 기본 PDF 게시에서 사용되는 템플릿의 **페이지 레이아웃** 도구 모음에서 **컨텐츠 편집** 옵션에 대해 잘못된 아이콘과 도구 설명이 표시됩니다. (13492)
- Experience Manager Guides **일괄 게시 대시보드**&#x200B;를 사용하여 사이트를 활성화할 수 없습니다. (13439)
...
- 편집기 UI를 통해 만든 폴더 프로필 수준 출력 사전 설정에 **ditaval** 옵션이 없습니다. (11903)
- **AEM 사이트** 사전 설정에서 **모든 주제에 대해 별도의 PDF을 생성**&#x200B;하는 옵션이 작동하지 않습니다. (11555)
...
- **기본 필터** 파일이 편집기에서 파일 이름으로 작동하지 않습니다. (13486)
- 편집기에서 베이스라인에 선택한 DITA 파일 버전 대신 이전 버전에 대한 제목이 표시됩니다. (13444)
...
- `labels.json` 파일의 레이블은 편집기에서 임의의 순서로 나타납니다. (10508)
...
- 편집기에서 이전 및 현재 버전의 나란히 표시되는 검토 패널에서는 대칭 문제가 발생합니다. (14156)
