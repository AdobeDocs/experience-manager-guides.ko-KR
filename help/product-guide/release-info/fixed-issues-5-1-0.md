...
- `XMLEditorConfig`에서 **ID 자동 생성** 옵션이 활성화된 경우에도 이러한 요소가 스니펫을 통해 추가되거나 템플릿을 통해 만들어지는 경우 요소에 대해 새 ID가 생성되지 않습니다. (GUIDES-21734)
...
- 주제 파일을 닫으면 주제가 잠긴 경우에도 새 버전으로 저장할 수 있습니다. 이 문제는 특히 `XMLEditorConfig`에서 **닫을 때 새 버전 묻기** 옵션이 활성화된 경우 발생합니다. (GUIDES-23875)
...
- codeblock 요소 내의 새 줄에 새 내용을 붙여 넣을 때 빈 줄이 자동으로 삽입됩니다(GUIDES-27842).
