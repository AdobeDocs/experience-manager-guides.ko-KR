...
1. 클라우드 서비스 Git 코드의 `/dox/dox.installer/pom.xml` 파일에서 `<dox.version>` 속성을 2022.1.78로 업데이트합니다.
...
* 소스와 작성자 모드 간을 전환하면 주제가 더티(dirty)로 표시되며 콘텐츠를 다시 저장해야 합니다(8524).
* 특수 문자 `[` 또는 `*`을(를) 검색할 때 콘텐츠 패널 충돌을 다시 사용합니다.(8279)
