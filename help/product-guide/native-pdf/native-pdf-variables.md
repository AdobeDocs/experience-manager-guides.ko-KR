...
![새 변수 만들기](assets/add-variable-default.png){width="800"}

*변수를 만들고 값을 정의합니다.*


1. 편집기에서 왼쪽 패널로 이동하여 **변수**&#x200B;를 선택합니다 <img alt= "변수 아이콘" src="./assets/variables-icon.svg" width="25">. 이 옵션은 기타 섹션에서 사용할 수 있습니다.
1. **편집** 선택 <img alt= "연필 아이콘 편집" src="./assets/edit_pencil_icon.svg" width="25">: **변수** 편집기를 엽니다.
변수는 알파벳순으로 나열됩니다.
1. **Name** 열에 변수 이름을 입력하고 **Value** 열에 해당 값을 입력합니다.
   >[!TIP]
   >
   >모든 HTML 콘텐츠를 변수 값으로 사용하여 변수 값을 특정 형식으로 표시할 수 있습니다. 예를 들어 변수 값에 `<b>` 태그를 추가하여 값 **Experience Manager Guides**&#x200B;을(를) 굵게 표시할 수 있습니다. 저장소의 이미지를 값으로 추가할 수도 있습니다.

1. **변수 추가** 선택 <img alt= "추가 아이콘" src="./assets/add-icon.svg" width="25">하여 새 변수를 추가합니다. 기존 변수와 이름이 같은 변수는 만들 수 없습니다. 오류가 표시됩니다.
...
변수 미리보기에는 기본값이 표시됩니다. 예를 들어 ProductName 변수의 기본값을 &quot;Adobe Experience Manager Guides&quot;로 정의한 경우 미리 보기에 이 값이 표시됩니다.
...
1. 변수 위로 마우스를 가져가 **옵션** 메뉴를 확인한 다음 **미리 보기** 옵션을 선택하십시오.
   ...
   변수 패널에서 ![변수 미리 보기](assets/variables-panel-preview-default.png){width="550"}
...
변수의 기본 이름은 `<selected variable name>`입니다(예: &quot;sample&quot;). 필요에 따라 이름을 변경할 수 있습니다.
...
Adobe Experience Manager Guides에서는 변수 대체 값을 할당할 수 있는 변수 세트도 지원합니다. 예를 들어, 한 회사는 A와 B 두 가지 상품을 판매할 수 있다. 각각 다른 사양을 가지고 있습니다. 이러한 사양에는 제품 이름, 버전 번호 및 릴리스 날짜가 포함될 수 있습니다. 브랜딩에는 다른 차이점이 있을 수 있습니다. 변수 세트를 사용하여 변수에 대해 다른 값 세트를 정의합니다. 출력을 생성할 때 적절한 변수 세트를 선택하고 필요한 출력을 생성합니다.
...
1. **설정** 선택 <img alt= "설정 아이콘" src="./assets/settings-icon.svg" width="25"> **변수 집합 구성** 대화 상자를 엽니다.
   ![변수 집합 구성](assets/configure-variable-set.png){width="550"}
...
변수 편집기에서 ![변수 미리 보기](assets/variables-editor-preview.png){width="550"}
...
제품 문서의 출력을 생성하는 동안 변수를 추가하여 휴대하고 쉽게 업데이트할 수 있습니다. 문서의 여러 페이지에 표시되는 페이지 레이아웃 내에 이러한 변수를 삽입할 수 있습니다. 예를 들어 페이지 레이아웃의 머리글 영역(또는 바닥글이나 본문 같은 다른 부분)에 나타나는 ProductName 변수를 추가할 수 있습니다.
...
![변수 집합 드롭다운](assets/output-preset-variable-dropdown.png){width="550"}

*PDF 출력을 생성하는 데 사용할 출력 사전 설정의 드롭다운에서 변수 집합을 선택합니다.*

>[!NOTE]
>
> 드롭다운에서 (기본값)을 선택하여 모든 변수의 기본값을 게시할 수도 있습니다.

선택한 변수 세트에 따라 변수 세트에 정의된 변수 값에 해당하는 출력을 가져옵니다. 예를 들어 변수 집합 *Adobe-set1*&#x200B;을(를) 선택하면 출력에는 이 집합에 정의된 변수 값이 표시됩니다.
