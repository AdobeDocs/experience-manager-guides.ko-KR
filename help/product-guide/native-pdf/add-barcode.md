...
수정 `layout.css` PDF 생성 중 바코드를 렌더링할 파일입니다. &#39;qrcode&#39; 및 &#39;pdf417&#39;과 같은 다양한 바코드 유형이 지원됩니다.  자세한 내용은 [바코드 유형](#barcode-types).
...
1. 바코드를 삽입할 위치에 다음 범위를 추가합니다.
   ...
   >[!NOTE]
   >
   > 에서 정의한 것과 동일한 클래스 이름 사용 `layout.css`.

1. 바꾸기 `<Sample barcode>` 바코드 스캐너로 읽을 값 포함..
...
에 있는 모든 메타데이터 사용 `<topicmeta>` 바코드로 표시할 DITA 맵의 요소입니다. 올바른 XPath를 사용해야 합니다. 예를 들어 다음을 추가할 수 있습니다 `<resourceid>` 다음에서 `<topicmeta>` DITA 맵의
