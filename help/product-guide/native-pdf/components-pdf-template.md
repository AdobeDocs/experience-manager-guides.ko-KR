...
DITA 주제에 추가된 초안 주석은 기본적으로 기본 PDF 출력에서 제외됩니다. 출력 템플릿의 콘텐츠 스타일시트에서 `draft-comment` 스타일에 의해 제어됩니다. 여기서 `display` 속성은 `none`(으)로 설정됩니다.

관리자는 출력에 초안 주석이 표시되도록 이 스타일을 업데이트할 수 있습니다.

1. **출력 템플릿** 패널에서 맵이 PDF 게시에 사용하는 템플릿을 엽니다.
2. **스타일시트**&#x200B;를 확장하고 **콘텐츠**&#x200B;를 두 번 클릭하여 콘텐츠 스타일시트를 엽니다.
3. **스타일** 패널에서 **초안 설명**&#x200B;을 찾아 선택합니다.

   목록이 길면 검색 필드를 사용하여 빠르게 찾을 수 있습니다.

4. **속성** 패널에서 **디스플레이** 속성의 값을 `none`에서 보이는 값(예: `block`, `inline-block`, `grid` 등)으로 변경합니다.

   ![콘텐츠 스타일시트에서 초안 주석 속성 구성](./assets/draft-comment-setting.png)

5. 스타일시트를 저장합니다.

>[!NOTE]
>
>이 스타일은 일반적으로 스타일이 지정된 콘텐츠에 초안 주석을 표시할지 여부를 제어합니다. **기본 PDF** 출력에 특별히 초안 주석을 포함하려면 기본 PDF 출력 사전 설정에서 **초안 주석 포함** 옵션도 활성화해야 합니다. 자세한 내용은 [기본 PDF 출력 사전 설정](../web-editor/native-pdf-web-editor.md)을 참조하세요. 생성된 PDF에 편집기에 표시되는 것과 동일하게 초안 주석이 표시되려면 두 설정이 모두 필요합니다.
...
브레이크 전후에 표시할 텍스트를 정의할 수 있습니다. 예를 들어 5페이지에서 테이블이 중단되고 **Text Before Break**&#x200B;에 대해 `<Continued on page %page-num%>`을(를) 정의합니다.  텍스트는 5페이지 하단에 &quot;6페이지에서 계속됨&quot;을 표시합니다.
  ...
  >[!NOTE]
  >
  > **중단 전 텍스트** 또는 **중단 후 텍스트** 필드에서 언어 변수를 사용하는 경우 전체 텍스트가 단일 언어 변수 내에 정의되어 있는지 확인하십시오. 이러한 필드의 언어 변수 외부에 추가된 텍스트나 변수는 렌더링되지 않습니다. 예를 들어, **Text After Break 필드**&#x200B;에서 `${lng:Continued-from-page} %page-num%`과(와) 같은 변수의 조합을 사용하는 대신 필드에서 `${lng:Continued-from-page}`만 사용하고 별도로 `Continued-from-page` 언어 변수의 값을 `Continued-from-page %page-num%`(으)로 설정합니다.
...
* **빈 페이지의 페이지 레이아웃**: 빈 페이지의 페이지 레이아웃을 지정할 수도 있습니다. 선택한 레이아웃이 모든 빈 페이지에 적용됩니다. 예를 들어 모든 빈 페이지에 대해 빈 페이지 레이아웃을 디자인한 경우 드롭다운 목록에서 **빈**&#x200B;을(를) 선택하면 문서의 모든 빈 페이지에 빈 페이지 레이아웃이 표시됩니다.
...
>[!NOTE]
>
> 이 설정은 해당 구성 요소가 아닌 섹션에만 적용됩니다.  예를 들어 **챕터 및 주제**&#x200B;에 대해 **이전 페이지** 옵션을 선택하면 **챕터 및 주제** 섹션이 이전 페이지와 병합됩니다. **일반** 설정에 따라 다양한 챕터 및 항목이 게시됩니다.예를 들어 **설정에서 새 챕터 시작**&#x200B;에서 **홀수 페이지**&#x200B;를 선택하면 홀수 페이지에서 끝나는 챕터 뒤에 빈 페이지가 삽입됩니다.
...
* **사용자 지정**: 미리 정의된 순서 대신 사용자 지정 페이지 순서를 정의합니다.
  ...
  * &quot;X&quot; — 문서의 모든 페이지입니다. &quot;1..-1&quot;과 같은 결과입니다.
...
* {figure}: 그림 번호에 상호 참조를 추가합니다. 그림용으로 정의한 자동 번호 스타일에서 숫자 번호를 선택합니다.  예를 들어 &quot;페이지 {page}의 {figure} 보기&quot;를 사용할 수 있습니다. 출력의 상호 참조에는 자동 생성된 그림 번호와 페이지 번호 &quot;5페이지의 그림 1 참조&quot;가 포함되어 있습니다.
