...
>[!NOTE]
>
> 2025.11.0 릴리스부터 편집기의 **저장소**&#x200B;는 더 이상 사용되지 않으며 **클라우드 서비스** 설치용 **탐색기**(으)로 대체됩니다. **온-프레미스** 설정의 경우 Experience Manager Guides 5.1 릴리스까지 편집기 인터페이스를 통해 저장소를 계속 보고 사용합니다.
...
>[!NOTE]
>
> 2025.11.0 릴리스부터 편집기의 저장소라는 용어가 클라우드 서비스 설정용 **탐색기**(으)로 바뀝니다. 온-프레미스 설정에서는 편집기에서 저장소를 계속 보고 액세스합니다.
...
1. **모두 바꾸기**&#x200B;를 선택하여 한 번에 검색된 모든 파일에서 검색된 모든 문자열을 지정된 대체 문자열로 바꿉니다. 모든 발생 항목이 교체되면 알림이 표시됩니다.
   ...
   >[!NOTE]
   >
   >**모두 바꾸기** 단추를 사용하려면 폴더 프로필 관리자 또는 시스템 관리자가 **Workspace 설정**(**준비 시**&#x200B;에 **설정**)의 **일반** 탭에서 **모두 바꾸기** 옵션을 선택해야 합니다.
...
1. **모두 바꾸기**&#x200B;를 선택하여 단일 파일에서 검색된 모든 검색어를 한 번의 이동으로 바꿀 검색어로 바꿉니다. 선택한 파일에서 모든 발생 항목을 바꾼 후 알림이 표시됩니다.
   ...
   >[!NOTE]
   >
   > then **모두 바꾸기** 아이콘을 활성화하려면 폴더 프로필 관리자 또는 시스템 관리자가 **Workspace 설정**(**준비 시**&#x200B;에 **설정** 표시)의 **일반** 탭에서 **모두 바꾸기** 옵션을 선택해야 합니다. 바꾸기 작업 중에 오류가 발생하면 XML 구문 분석 문제 또는 DITA 관련 오류로 인해 해당 파일을 건너뜁니다.
