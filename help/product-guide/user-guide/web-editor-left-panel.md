...
>[!NOTE]
>
> 관리자는 편집기에서 파일 이름별로 파일 목록을 보도록 선택할 수도 있습니다. **사용자 환경 설정**&#x200B;의 **편집기 파일 표시 구성** 섹션에서 **파일 이름** 옵션을 선택하십시오.
...
![고급 필터 대화 상자](images/repository-filter-search-advanced.png) {width="650"}
...
1. **만들기**&#x200B;를 선택합니다. 지정한 경로에 주제가 만들어집니다. 또한 편집기에서 편집할 주제가 열립니다.
...
- **에셋 업로드**: 로컬 시스템에서 Adobe Experience Manager 저장소의 선택한 폴더로 파일을 업로드합니다. 로컬 시스템에서 현재 작업 주제로 파일을 드래그 앤 드롭할 수도 있습니다. 로컬 시스템의 이미지를 주제에 삽입하려는 경우 매우 유용합니다.
  ...
  업로드가 완료되면 다음과 같은 확인 대화 상자가 표시됩니다.

  ![](images/upload-successful.png)

  <details>

  <summary>파일 이름의 잘못된 문자 처리</summary>

  업로드되는 에셋의 파일 이름에 잘못된 문자(예: * / : [\])가 포함된 경우 | # % { } ?), 다음 시나리오가 발생할 수 있습니다.

  - **부분 업로드**: 업로드되는 하나 이상의 에셋에 파일 이름에 잘못된 문자가 포함되어 있음을 나타냅니다.

    ![](images/partial-asset-upload.png)

  - **업로드 실패** : 업로드되는 모든 에셋의 파일 이름에 잘못된 문자가 포함되어 있음을 나타냅니다.

    ![](images/upload-asset-failed.png)

  이러한 충돌을 해결하려면 에셋의 파일 이름에서 잘못된 문자를 제거한 다음 저장소에 다시 업로드하십시오.
...
- **자산 재처리**: 폴더에 대한 모든 자산의 처리를 트리거합니다.
...
- **속성**: 선택한 파일의 속성 페이지를 여는 데 사용합니다.
  ...
  이 페이지에서 메타데이터 속성을 추가, 삭제 또는 수정하면(기본 또는 사용자 지정) 문서 버전에서 [작업 복사본 표시기](./web-editor-edit-topics.md#working-copy-indicator)가 트리거됩니다.

  파일을 선택한 다음 도구 모음에서 속성 아이콘을 선택하여 Assets UI에서 속성 페이지에 액세스할 수도 있습니다.
...
그러나 유용성과 효율성을 개선하도록 설계된 향상된 검색 및 필터링 경험을 도입했습니다.

파일 및 폴더의 상황에 맞는 메뉴에 대한 자세한 내용은 [옵션 메뉴](#options-menu)를 참조하세요.

검색 경험에 대한 자세한 내용은 [검색 패널](./search-panel-explorer.md)을 참조하세요.
...
- **탐색기에서 찾기**: 탐색기\(또는 DAM\)에서 맵 파일의 위치를 표시합니다.
...
- **상태 검사 실행**: 선택한 맵에서 상태 검사를 실행하려면 이 옵션을 사용합니다. 이 기능에 대한 자세한 내용은 [맵 편집기의 추가 기능](./map-editor-other-features.md#run-health-check-on-a-map)을 참조하세요.
  >[!NOTE]
  >
...
- **탐색기에서 찾기**: 탐색기 \(또는 DAM\)에서 선택한 파일의 위치를 표시합니다.
...
- 재사용 가능한 콘텐츠 옆의 **+** 아이콘을 선택하여 **파일 선택** 대화 상자를 엽니다.
- 추가할 파일을 선택한 다음 **선택**&#x200B;을 선택합니다. 필터 검색 옵션을 사용하여 특정 파일을 검색할 수도 있습니다. 자세한 내용은 [편집기의 다른 기능](./web-editor-other-features.md)을 참조하세요.
...
관리자가 속성에 대한 프로필을 만든 경우 해당 속성과 구성된 값을 가져옵니다. **Workspace 설정**&#x200B;의 **표시 특성** 탭에서 관리자가 구성한 표시 특성(**버전 5.2** 이전의 On-Premise 설정에 대해 **설정**)을 할당할 수도 있습니다. 요소에 대해 정의된 속성이 레이아웃 및 아웃라인 뷰에 표시됩니다.
...
1. 파일을 .ditamap 확장명으로 저장하고 DAM의 폴더에 업로드합니다.
...
조건 패널에서 제목 구성표를 보려면 시스템 관리자는 **Workspace 설정**&#x200B;의 일반 탭에 있는 **조건 패널에 주제 구성표 표시** 옵션을 선택해야 합니다(**이전 버전 5.2**&#x200B;의 On-Premise 설정에 대해 **설정** 표시). 자세한 내용은 [탭 모음](./web-editor-tab-bar.md)을 참조하세요.
...
1. 코드 조각 옆의 **+** 아이콘을 선택하여 **새 코드 조각** 대화 상자를 엽니다.

   ![](images/snippet-new-dialog.png){width="300"}

   새 코드 조각 대화 상자에서 [코드 조각] 패널에 나타나는 제목, 설명, 콘텐츠의 형식(DITA 또는 HTML) 선택 및 만들려는 코드 조각 콘텐츠의 코드를 제공합니다. 코드 조각을 저장하고 만들려면 **만들기**&#x200B;를 선택하십시오.

2. 콘텐츠 편집 영역에서 코드 조각으로 사용할 요소의 이동 경로를 마우스 오른쪽 단추로 클릭하고 컨텍스트 메뉴에서 **코드 조각 만들기**&#x200B;를 선택합니다. 선택한 요소의 XML 코드가 **Content** 필드에 채워진 새 코드 조각 대화 상자가 나타납니다. 코드 조각에 대한 **제목** 및 **설명**&#x200B;을 입력하고 **만들기**&#x200B;를 선택하여 코드 조각을 저장합니다.

3. 콘텐츠 편집 영역에서 코드 조각으로 사용할 콘텐츠의 아무 곳이나 마우스 오른쪽 단추로 클릭하고 상황에 맞는 메뉴에서 **코드 조각 만들기**&#x200B;를 선택합니다. 선택한 요소의 XML 코드가 **Content** 필드에 채워진 새 코드 조각 대화 상자가 나타납니다. 코드 조각에 대한 **제목** 및 **설명**&#x200B;을 입력하고 **만들기**&#x200B;를 선택하여 코드 조각을 저장합니다.
...
- [코드 조각] 패널에서 코드 조각을 선택하고 주제의 원하는 위치에 드래그 앤 드롭합니다. 또한 [코드 조각] 패널 상단의 필터 옵션을 사용하여 보기를 세분화할 수 있습니다.

  - **모든 코드 조각 표시**: DITA 및 HTML 형식을 포함하여 사용 가능한 코드 조각을 모두 나열합니다.
  - **적용 가능한 코드 조각만 표시**: 현재 주제나 컨텍스트와 관련된 코드 조각만 표시하도록 목록을 필터링합니다. 예를 들어 DITA 주제를 작업하는 경우 컨텍스트 정확성을 보장하기 위해 HTML 스니펫이 목록에서 제외됩니다.
...
찾기 및 바꾸기 기능은 요구 사항에 따라 두 가지 모드에서 사용할 수 있습니다.

- **Source 모드 사용:** 검색 시 검색된 문자열의 표시 컨텐츠뿐만 아니라 기본 소스 컨텐츠(요소, 태그 및 특성 값을 포함한 XML 구조)도 검색됩니다. 이 모드에서는 전체 콘텐츠를 포괄적으로 검색할 수 있습니다. 이 기능을 사용하려면 [찾기 및 바꾸기] 패널에서 사용할 수 있는 **원본 모드 사용** 옵션을 활성화해야 합니다.

  >[!NOTE]
  >
  > **소스 모드 사용** 기능은 Experience Manager Guides as a Cloud Service(2026.01.0 릴리스 포함) 및 온-프레미스 설정(5.2 릴리스 포함)에서 사용할 수 있습니다. 이 기능을 사용하려면 먼저 사용 중인 설정에 따라 콘텐츠를 색인화해야 합니다.
  > - **클라우드 서비스의 경우**: 사용자 지정 인덱스 배포를 수행해야 합니다. 자세한 내용은 [사용자 지정 인덱스 배포](/help/product-guide/cs-install-guide/custom-indexing.md)를 참조하세요. 완료되면 고객 지원 팀에 문의하여 이 기능을 활성화하십시오.
  > - **온-프레미스용**: 기능을 사용하려면 기존 콘텐츠를 다시 인덱싱해야 합니다. 자세한 내용은 [콘텐츠 다시 인덱싱](/help/product-guide/install-guide/custom-indexing-prem.md)을 참조하십시오. 완료되면 고객 지원 팀에 문의하여 이 기능을 활성화하십시오.
...
<details>
    <summary> 글로벌 검색을 수행하고 Source 모드로 바꾸기</summary>

글로벌 검색을 수행하고 Source 모드로 대체하려면 다음 단계를 수행하십시오.

1. 전역 **찾기 및 바꾸기** 패널을 엽니다.
1. **원본 모드 사용** 옵션을 사용하도록 설정합니다.
1. **경로** 드롭다운을 선택하고 다음 옵션 중 하나를 선택하여 검색을 수행합니다.

   - **경로**: 선택한 경로에서 검색하려면
   - **맵**: 리포지토리 또는 컬렉션의 일부 특정 맵에서 검색하려면
   - **현재 맵**: 현재 열려 있는 맵에서 검색

   ![](images/path-dropdown-source-mode.png){width="350"}


1. **찾기** 필드에 검색 문자열을 입력합니다. 결과 범위를 좁히려면 [찾기] 필드 옆에 있는 **필터** 아이콘을 선택하고 원하는 다음 필터를 선택합니다.

   ![](images/find-filters-source-mode.png){width="350"}

   - **파일 형식**: 텍스트를 검색할 파일 형식 선택; **항목** 및 **맵**
   - **문서 상태**: 사용 가능한 옵션에서 문서 상태를 선택할 수 있습니다. 표시되는 문서 상태 옵션은 폴더 프로필에서 파생됩니다. 해당 프로필에서 사용 가능한 모든 문서 상태의 결합된 세트를 나타냅니다. 기본 상태에는 [검토 중], [완료], [초안], [승인됨], [편집] 및 [검토됨]이 포함됩니다.
   ...
   - **기타**: 다음을 구성할 수 있습니다.
     - **태그**: 태그를 기반으로 콘텐츠를 필터링합니다.
     - **대/소문자를 구분하는 검색**: 지정된 대/소문자와 일치하는 결과를 검색할 수 있도록 설정합니다.
     - **다른 사용자가 잠근 파일 나열**: 다른 사용자가 현재 잠근 파일을 표시하여 잠금이 해제될 때까지 편집할 수 없습니다.

1. Enter 키를 누르거나 **검색** 아이콘을 선택하여 검색을 수행합니다.

   ![](images/search-icon.png){width="350"}

1. 검색 결과 목록에서 파일을 선택합니다. 검색된 용어가 콘텐츠에서 강조 표시된 상태로 Source 보기에서 파일이 열립니다.

1. **다음으로 바꾸기** 필드에 대체 용어로 사용할 용어를 입력하십시오. 대체 요소 적용 방법을 사용자 지정하려면 필드 옆에 있는 **설정** 아이콘을 클릭하고 사용 가능한 옵션 중에서 선택합니다.

   - **잠금 해제된 파일 바꾸기**: 잠금 해제된 파일에서 바꾸기를 허용하려면 이 옵션을 선택하십시오.

   - **바꾸기 후 새 버전을 만듭니다**: 텍스트를 바꾸도록 선택한 항목의 새 버전을 만들려면 이 옵션을 선택합니다. 업데이트된 각 파일과 함께 추가될 버전 설명을 제공할 수도 있습니다. 이 옵션을 선택하지 않으면 변경 사항이 주제의 현재 버전에 저장되고 새 버전이 만들어지지 않습니다.

   ![](images/replace-settings-source-mode.png){width="350"}


1. **발생 항목 바꾸기**&#x200B;를 선택하여 주제의 현재 강조 표시된 검색 문자열을 바꾸거나, 위쪽 및 아래쪽 화살표를 선택하여 텍스트의 다음 또는 이전 발생 항목으로 이동합니다.

   ![](images/replace-occurrence.png){width="350"}

1. **모두 바꾸기**&#x200B;를 선택하여 한 번에 검색된 모든 파일에서 검색된 모든 문자열을 지정된 대체 문자열로 바꿉니다. 모든 발생 항목이 교체되면 알림이 표시됩니다.

   ![](images/replace-all.png){width="350"}

   >[!NOTE]
   >
   >**모두 바꾸기** 단추를 사용하려면 폴더 프로필 관리자 또는 시스템 관리자가 **Workspace 설정**&#x200B;의 **일반** 탭에서 **모두 바꾸기** 옵션을 선택해야 합니다(**버전 5.2** 이전의 On-Premise 설정에 대해 **설정** 표시).

1. 검색 결과 목록의 파일 위로 마우스를 가져가면 오른쪽에 있는 **파일에서 모두 바꾸기** 아이콘을 볼 수 있습니다. 이 아이콘을 사용하면 단일 파일에서 모든 항목을 바꿀 수 있습니다.

   >[!NOTE]
   >
   > 검색 결과에서 파일을 제거하려면 **제거** 아이콘도 가져옵니다. 제거한 파일이 목록에서 이동되고 검색된 용어가 대체되지 않습니다.

   ![](images/replace-all-in-file.png){width="350"}

1. 모두 바꾸기 작업이 완료되면 다운로드 가능한 CSV 보고서가 생성되고, 수행된 모든 바꾸기 작업에 대한 스냅숏이 제공됩니다. 보고서를 다운로드하여 성공적으로 교체된 발생 횟수, 각 실패와 해당 원인에 대한 세부 정보와 함께 교체 작업에 대한 세부 정보를 볼 수 있습니다. 다른 사용자가 파일을 잠근 경우, 작업 중 파일의 변경 내용으로 인한 유효성 검사 오류 또는 기타 유사한 문제와 같은 몇 가지 특정 이유로 인해 작업이 실패할 수 있습니다.

   ![](images/snapshot-replace-all.png){width="350"}

전체 시스템에서 한 번에 하나의 모두 바꾸기 작업만 수행할 수 있으며 작업이 수행될 때까지 &quot;모두 바꾸기 진행 중&quot; 상태를 볼 수 있습니다. 그 사이에 있는 모두 바꾸기 작업을 중단할 수도 있습니다. 작업을 중단하면 받은 편지함에 작업에 대한 알림이 표시됩니다.
...
- **Source 모드가 없는 경우:** XML 요소 또는 특성과 같은 원본 콘텐츠를 건너뛰고 표시되는 콘텐츠에만 중점을 두고 작성자 보기에 표시된 텍스트만 검색하도록 제한됩니다. 이 모드는 빠른 콘텐츠 전용 검색에 이상적입니다.

  ![](images/map-find-replace-without-source-mode.png)

<br>
<details>
    <summary> Source 모드 없이 글로벌 검색 및 바꾸기</summary>


Source 모드 없이 글로벌 검색을 수행하고 바꾸려면 다음 단계를 수행하십시오.
...
1. 검색 결과 목록에서 파일을 선택합니다. 검색된 용어가 콘텐츠에서 강조 표시된 상태로 파일이 콘텐츠 편집 영역의 작성자 보기에서 열립니다.

1. **다음으로 바꾸기** 필드에 대체 용어로 사용할 용어를 입력하십시오. 대체 요소 적용 방법을 사용자 지정하려면 필드 옆에 있는 **설정** 아이콘을 클릭하고 사용 가능한 옵션 중에서 선택합니다.

   - **바꾸기 전에 파일 잠금**: 검색 문자열을 바꾸기 전에 파일을 자동으로 잠그려면 이 옵션을 선택하십시오. 이 설정은 관리자가 편집하기 전에 파일을 잠그도록 구성을 활성화한 경우에 더 관련이 있습니다. 백엔드 설정이 활성화된 상태에서 이 옵션을 선택해야 합니다. 이 옵션을 사용하면 변경하기 전에 모든 파일을 잠그라는 메시지가 표시되지 않습니다. 이 옵션을 선택하지 않으면 편집을 위해 파일을 열기 전에 프롬프트가 나타납니다.
...
1. **모두 바꾸기**&#x200B;를 선택하여 단일 파일에서 검색된 모든 검색어를 한 번의 이동으로 바꿀 검색어로 바꿉니다. 선택한 파일에서 모든 발생 항목을 바꾼 후 알림이 표시됩니다.

   >[!NOTE]
   >
   > then **모두 바꾸기** 아이콘을 활성화하려면 폴더 프로필 관리자 또는 시스템 관리자가 **Workspace 설정**&#x200B;의 **일반** 탭에서 **모두 바꾸기** 옵션을 선택해야 합니다(**버전 5.2** 이전의 On-Premise 설정에 대해 **설정** 표시). 바꾸기 작업 중에 오류가 발생하면 XML 구문 분석 문제 또는 DITA 관련 오류로 인해 해당 파일을 건너뜁니다.
