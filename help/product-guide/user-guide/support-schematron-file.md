...
1. Schematron 아이콘을 선택하여 Schematron 패널을 엽니다.
...
1. 추가된 Schematron 파일을 사용하여 항목의 유효성을 검사하려면 **유효성 검사**&#x200B;를 선택하십시오.
...
## 유효성 검사 결과 및 심각도 수준 이해

검증 결과는 Schematron 파일에 정의된 역할 속성에 따라 표시됩니다. 문제는 `Fatal`, `Error`, `Warn` 또는 `Info`(으)로 분류되며 유효성 검사 패널에 각 범주에 대한 항목이 표시됩니다.

![](images/schematron-validation-errors.png){width="350" align="left"}

문제의 심각도를 확인하기 위해 해당 Schematron 파일에 정의된 역할 특성의 _대/소문자 구분_ 값을 평가합니다.

다음 코드 조각은 Schematron 규칙에 정의된 지원되는 역할 속성 값을 보여 줍니다.
...
역할 특성이 지정되지 않았거나 지원되지 않는 값을 사용하는 경우 문제가 유효성 검사 패널에서 `Error`(으)로 분류됩니다. 이 동작은 역할 특성을 정의하지 않는 기존 Schematron 파일에도 적용됩니다. 이러한 경우 모든 문제는 `Error` 아래에 그룹화됩니다.

**파일 저장 시나리오**

파일을 저장하는 것은 **Workspace 설정**&#x200B;의 [파일을 저장하기 전에 유효성 검사 실행](../cs-install-guide/workspace-settings.md#validation) 설정에 따라 다릅니다.

* 사용하도록 설정하면 `Fatal` 또는 `Error` 수준 문제가 해결되지 않을 때까지 파일을 저장할 수 없습니다.
* 비활성화하면 유효성 검사가 수행되지 않으며 `Fatal` 또는 `Error` 수준의 문제가 있더라도 파일을 저장할 수 있습니다.
