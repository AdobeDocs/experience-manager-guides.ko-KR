...
**주제 목록** 탭이 레거시 매핑을 기반으로 만들어지지 않은 AEM 사전 설정에 있습니다.
...
| Publish 경로 | 출력이 저장되는 AEM 저장소 내의 경로입니다. Publish 경로는 홈 페이지 템플릿을 기반으로 생성된 페이지를 포함하는 모든 경로로 채워집니다. DITA 맵의 AEM Sites 출력이 이 경로 아래에 생성됩니다.  예를 들어 사이트를 `AEMG-Docs`(으)로 지정하고 Publish 경로를 `aemg-docs-en/docs/product-abc.`(으)로 지정하면 AEM Sites 출력이 `crx/de`의 `aemg-docs-en/docs/product-abc/` 노드 아래에 생성됩니다. |
...
| 메타데이터 <br> <br>맵 속성을 대체 항목으로 사용 | 선택하면 맵 파일에 대해 정의된 속성도 이러한 속성이 정의되지 않은 항목에 복사됩니다. 이 옵션을 사용하는 동안 다음 사항을 고려하십시오.<br><br>*문자열, 날짜 또는 긴(단일 및 다중 값) 속성만 AEM Site 페이지에 전달할 수 있습니다.<br>* String 형식 속성에 대한 메타데이터 값이 특수 문자(예: `@, #, " "`)를 지원하지 않습니다.<br>* 이 옵션은 `Properties` 옵션과 함께 사용해야 합니다. |
...
1. 다음 두 가지 방법으로 기본 제공 사이트 템플릿을 구성할 수 있습니다.
   1. **사이트**&#x200B;를 선택한 다음 채워진 옵션에서 게시 경로와 주제 페이지 템플릿을 선택합니다.
      ...
      1. **Publish 경로** 및 **주제 페이지 템플릿** 옵션이 드롭다운에 자동으로 설정됩니다. 옵션을 선택할 수도 있습니다. 예를 들어 `AEMG-Docs-Site/en/docs/product1`과(와) `Topic page`이(가) 각각 설정됩니다.
   1. 전체 사이트 경로 선택:
