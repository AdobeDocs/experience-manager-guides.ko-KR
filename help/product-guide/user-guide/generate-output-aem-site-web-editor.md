...
| AEM Sites 옵션 | 설명 |
| --- | --- |
| 사이트 이름 | 출력이 AEM 저장소에 저장되는 사이트 이름입니다.<br><br>AEM 저장소의 노드가 여기에 지정된 이름으로 만들어집니다. 사이트 이름을 지정하지 않으면 사이트 노드가 DITA 맵 파일 이름으로 생성됩니다.<br><br>여기에서 지정한 사이트 이름은 브라우저 탭의 제목으로도 사용됩니다.<br><br>사이트 이름을 설정하는 동안 변수를 사용할 수도 있습니다. 자세한 내용은 [대상 경로, 사이트 이름 또는 파일 이름 옵션을 설정할 변수 사용](./generate-output-use-variables.md)을 참조하세요. |
