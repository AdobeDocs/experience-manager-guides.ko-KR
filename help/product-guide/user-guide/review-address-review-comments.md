...
이 기능을 사용하면 [주석] 패널에서 현재 열려 있는 주제와 연관된 검토 작업(열거나 닫힘)을 선택할 수 있습니다. 활성화되면 각 작업의 상태(활성 또는 마감됨) 및 해당 작업이 속한 프로젝트와 함께 주제가 속한 모든 검토 작업이 나열된 드롭다운 목록이 검토 UI의 주석 패널에 포함됩니다. 현재 작업 중인 작업은 기본적으로 **현재** 태그로 표시됩니다.

![](./images/review-task-selection-dropdpwn.png){width="350"}

검토 작업 드롭다운을 사용하여 검토 프로젝트를 전환하지 않고도 이러한 작업을 선택하고 해당 설명을 진행할 수 있습니다. 드롭다운에서 작업 옆에 있는 **(i)** 아이콘을 선택하여 작업 세부 정보를 볼 수도 있습니다.

>[!NOTE]
>
> 보려는 검토 작업이 드롭다운에 표시되지 않으면 인스턴스에서 이 기능을 활성화하기 전에 작업이 생성되었기 때문일 수 있습니다. 관리자에게 문의하여 문제를 해결하려면 [색인화를 수행하여 [주석] 패널의 항목에 대한 모든 검토 작업을 포함합니다](../install-conf-guide/index-review-tasks.md).

이 기능을 사용할 때는 다음 사항에 유의하십시오.

- 편집기에서 다른 주제로 전환하면 패널이 주석의 현재 작업으로 다시 재설정됩니다
- 현재 작업 이외의 작업을 선택하면 해당 작업의 주제와 주석 버전이 읽기 전용 모드로 열립니다. 댓글을 볼 수는 있지만 편집, 수락 또는 거부할 수는 없으며 **가져오기** 옵션을 사용할 수 없습니다. 현재 작업의 주석만 작업 복사본으로 가져올 수 있습니다.
- 필터링 및 검색은 선택한 작업의 주석에만 적용되며, 선택하지 않은 다른 작업의 주석은 영향을 받지 않습니다.
