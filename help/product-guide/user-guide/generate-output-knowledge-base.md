---
title: 기술 자료
description: 맵 콘솔에서 기술 자료 사전 설정을 만드는 방법을 알아봅니다. AEM Guides에서 기술 자료 출력 사전 설정을 구성합니다.
feature: Publishing
role: User
...
맵 콘솔에서 **기술 자료** 사전 설정을 만들려면 다음 단계를 수행하십시오.
...
1. **새 출력 사전 설정** 대화 상자의 유형 드롭다운에서 **기술 자료**&#x200B;를 선택합니다.
1. **대상** 필드에서 생성된 출력의 대상을 선택합니다. 사용 가능한 옵션은 **Adobe Experience Manager**, **Salesforce** 및 **ServiceNow**&#x200B;입니다.
...
기술 자료 사전 설정 구성 옵션은 **일반**, **문서** 및 선택한 대상(**AEM**/ **ServiceNow**/ **Salesforce**) 탭에 구성됩니다.

![](./images/kb-aem-preset.png){width="550"}

### 일반

**일반** 탭에서 다음 구성 옵션을 사용할 수 있습니다.

| 기술 자료 옵션 | 설명 |
| --- | --- |
| 다음을 사용하여 조건 적용 | 다음 옵션 중 하나를 선택합니다.<br><br>* **적용되지 않음**: 게시된 출력에 조건을 적용하지 않으려면 이 옵션을 선택합니다.<br>* **DITAVAL 파일**: DITAVAL 파일을 선택하여 개인화된 콘텐츠를 생성합니다. 찾아보기 대화 상자를 사용하거나 파일 경로를 입력하여 여러 DITAVAL 파일을 선택할 수 있습니다. 파일 이름 근처에 있는 교차 아이콘을 사용하여 제거합니다. DITAVAL 파일은 지정된 순서대로 평가되므로 첫 번째 파일에 지정된 조건이 이후 파일에 지정된 조건에 우선합니다. 파일을 추가하거나 삭제하여 파일 순서를 유지할 수 있습니다. DITAVAL 파일을 다른 위치로 이동하거나 삭제하는 경우 사전 설정에서 자동으로 삭제되지 않습니다. 파일이 이동되거나 삭제될 경우에 대비하여 위치를 업데이트해야 합니다. 파일 이름 위로 마우스를 가져가면 파일이 저장된 Adobe Experience Manager 저장소의 경로를 볼 수 있습니다. DITAVAL 파일만 선택할 수 있으며 다른 파일 형식을 선택하면 오류가 표시됩니다.<br><br> **참고**: **Salesforce 게시**&#x200B;에 대해 DITAVAL 필터링을 사용할 때는 다음 사항을 고려하십시오. <br> - 각 DITAVAL 속성에 대해 `Include` 및 `Exclude` 작업만 지원됩니다.<br> - 출력에서 조건부 콘텐츠를 시각적으로 표시하거나 강조 표시하는 플래그 지정이 지원되지 않습니다.<br> - 출력 사전 설정에서 단일 DITAVAL 파일만 게시하도록 선택할 수 있습니다. 여러 DITAVAL 파일 선택은 Salesforce 게시에서 지원되지 않습니다. 콘텐츠 내의 <br>- `ditavalref` 참조는 지원되지 않습니다. <br><br> **조건 사전 설정**: 출력을 게시하는 동안 드롭다운에서 조건 사전 설정을 선택하여 조건을 적용합니다. DITA 맵 콘솔의 조건 사전 설정 탭에 있는 조건을 추가한 경우 옵션이 표시됩니다. 조건 사전 설정에 대해 자세히 알아보려면 [조건 사전 설정 사용](generate-output-use-condition-presets.md#id1825FL004PN)을 확인하십시오. |
...
| 사후 생성 워크플로 | 이 옵션을 선택하면 Adobe Experience Manager에 구성된 모든 워크플로우가 포함된 새로운 게시물 생성 워크플로우 드롭다운 목록이 표시됩니다. 출력 생성이 완료된 후 실행할 워크플로를 선택해야 합니다.<br><br>**참고**: Cloud Services용 설치 및 구성 가이드의 [출력 후 생성 워크플로 사용자 지정](../cs-install-guide/customize-workflows.md#id17A6GI004Y4) 섹션에 대해 자세히 알아보세요. |
...
구성 옵션은 선택한 대상에 따라 변경됩니다.


**Adobe Experience Manager**

**Adobe Experience Manager**&#x200B;에 대해 다음 구성 옵션이 대상으로 표시됩니다.


>[!NOTE]
>
>Adobe Experience Manager 기술 자료 사전 설정은 관리자가 구성한 경우에만 사용할 수 있습니다.
...
>[!TIP]
> 
>**새로 고침** 아이콘을 선택하여 선택한 기술 자료 템플릿에 따라 필드에 있는 각 템플릿을 채웁니다.

**ServiceNow**

**ServiceNow**&#x200B;에 대해 다음 구성 옵션이 대상으로 표시됩니다.
...
**Salesforce**&#x200B;에 대해 다음 구성 옵션이 대상으로 표시됩니다.
