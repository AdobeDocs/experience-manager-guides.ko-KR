...
* [기준선을 사용하여 콘텐츠 미리보기](#preview-content-using-baseline)
* [조건부 필터를 기반으로 콘텐츠 보기](#id2114BI00VXA)
* [변경 내용 추적 마크업 보기](#id2114BJ00CE8)
* [주제를 PDF으로 내보내기](#id2114BL00B5U)


### 기준선을 사용하여 콘텐츠 미리보기(DITA 맵에만 적용 가능)

>[!NOTE]
>
>귀하의 환경에서 이 기능을 활성화하려면 고객 지원 팀에 문의하십시오.

기본적으로 미리보기 모드에서는 주제 또는 맵의 현재 작업 복사본이 표시됩니다. 맵에 하나 이상의 정적 기준선이 있는 경우 **기준선을 사용하여 미리 보기** 전환을 사용하여 특정 기준선에 있는 맵 대신 미리 볼 수 있습니다. 이 기능은 정적 기준선만 지원하며, 동적 기준선은 미리보기에 사용할 수 없습니다.

![](images/preview-baseline.png){width="650"}

드롭다운 목록에서 기준 요소를 선택하면 미리 보기가 현재 작업 복사본 대신 선택한 기준 요소를 기반으로 모든 관련 항목, 에셋, 이미지 및 참조를 확인합니다. 이렇게 하면 기준선이 생성된 시점의 맵 콘텐츠를 정확하게 표현할 수 있습니다.

예를 들어 기준선이 생성된 후 이미지가 교체된 경우 해당 기준선을 미리 보면 현재 작업 복사본의 이미지가 아니라 기준선과 연관된 이미지의 버전이 표시됩니다.

미리보기 모드는 초기 주제 세트를 렌더링하는 동안 로드되는 미리보기 표시기를 표시합니다. 항목을 편집하거나 버전을 전환한 후에는 자동으로 업데이트되지 않습니다. 최신 콘텐츠를 보려면 미리 보기를 수동으로 새로 고치거나 주제 또는 맵을 닫았다가 다시 엽니다. 또한 베이스라인에 대한 맵을 미리 볼 때 [추적] 옵션(마크업 없음, 원본 및 마크업 표시)을 사용할 수 없습니다.
...
다음 이미지는 `Audience`과(와) `Platform` 두 조건을 사용하는 주제를 보여 줍니다. 조건화된 콘텐츠는 노란색 배경으로 강조 표시됩니다.
...
* **원본**: 이 보기에서 모든 삽입이 거부되고 모든 삭제 내용이 다시 복원되며 미리 보기가 표시됩니다. 변경 내용 추적 모드를 활성화하기 전에 문서의 원본 양식을 가져옵니다.
