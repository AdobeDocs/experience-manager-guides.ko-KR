...
1. **할당 대상** 드롭다운 필드에서 검토할 항목을 보낼 검토자를 선택합니다. 을 입력할 때 선택한 프로젝트 내에서 멤버 이름이나 역할 이름으로 검색할 수도 있습니다.

   *할당 대상* 필드는 *프로젝트* 필드에서 활성 프로젝트를 선택한 후에만 사용할 수 있습니다. 선택한 프로젝트를 기반으로 구성원 또는 역할 목록을 가져옵니다. 프로젝트를 선택하지 않으면 *할당 대상* 필드가 비활성화됩니다.
   ...
   >[!NOTE]
   >
   > 검토 워크플로우는 프로젝트별로 다릅니다. 프로젝트를 만들 때 팀 구성원을 프로젝트에 추가하고 그룹에 할당합니다. 따라서 여기에서 프로젝트를 선택하면 해당 프로젝트에 속한 멤버를 선택할 수 있습니다. 프로젝트에 대한 자세한 내용은 [DITA 프로젝트 만들기](authoring-create-dita-project.md#)를 참조하십시오.
...
1. **자동 미리 알림**&#x200B;을(를) 활성화하여 검토자에 대한 예약된 미리 알림(및 이메일 미리 알림)을 구성합니다. 검토 작업의 기한 이전과 기한이 지난 후 모두 이 작업을 수행할 수 있습니다.

   >[!NOTE]
   >
   >자동 미리 알림 전환은 검토 작업에 대한 기한을 선택할 때까지 비활성화 상태로 유지됩니다.

   - **기한 전 미리 알림**: 미리 검토자에게 알리기 위해 기한 전 최대 60일까지 여러 미리 알림(최대 3개)을 구성할 수 있습니다. 미리 알림 1은 오늘부터 기한까지 남은 일 수로 설정할 수 있으며 미리 알림 값은 기한 이전 일 수를 나타내는 내림차순 뒤에 와야 합니다.

   - **기한 후 미리 알림**: 기한이 지난 후 미리 알림(최대 3개)을 구성하여 검토자에게 작업이 기한 초과되었음을 알릴 수도 있습니다. 기한 초과 미리 알림은 기한 이후 최대 30일까지 구성할 수 있습니다. 미리 알림 값은 기한 이후의 일 수를 나타내는 오름차순 순서여야 합니다.

   구성된 일정에 따라 시스템은 계획된 미리 알림 날짜에 AEM 알림 및 이메일 미리 알림을 검토자에게 보냅니다.
...
1. **다음**&#x200B;을 선택합니다.
   ...
   >[!NOTE]
   >
   > **다음** 단추는 *제목*, *프로젝트*, *할당 대상*, *기한*&#x200B;을 포함하여 모든 필수 필드에 대한 세부 정보를 제공한 후에만 사용할 수 있습니다.
...
검토 작업의 작성자 또는 개시자는 검토 작업 과정에 영향을 주지 않고 진행 중인 검토 작업에 새 주제를 추가하거나(이전에 검토를 위해 전송되지 않은 경우) 진행 중인 검토 작업에서 주제를 제거할 수 있습니다.

다음 단계를 수행하여 진행 중인 검토 작업에서 주제를 추가하거나 제거합니다.

1. [검토] 패널에서 수정할 검토 작업으로 이동하여 컨텍스트 메뉴에서 **작업 세부 정보 열기**&#x200B;를 선택합니다.

   **작업 세부 정보** 페이지가 표시됩니다.

1. 현재 검토 작업에 포함된 주제를 보려면 **콘텐츠** 탭을 선택하십시오.
1. 주제 목록을 검토하고 필요에 따라 선택 항목을 수정합니다.

   - 이전에 검토를 위해 전송되지 않은 새 주제를 선택합니다.
   - 검토 작업에서 제거할 주제를 선택 취소합니다.

   >[!NOTE]
   >
   > 진행 중인 검토 작업을 업데이트하려면 하나 이상의 주제를 선택한 상태로 유지해야 합니다.

   ![](images/modify-review-topics.png)

1. 주제 목록을 수정하면 해당 업데이트가 작업 세부 정보 페이지의 **검토자** 탭에 자동으로 전파됩니다.

   ![](images/modify-reviewers.png)

   검토자 탭에서 업데이트된 주제 목록을 검토합니다. 새로 추가한 항목의 경우 검토 작업에 대해 **피할당자의 항목 검토 허용** 설정이 사용되지 않는 한 검토자를 할당해야 합니다.
1. **업데이트**&#x200B;를 선택합니다.

검토 작업이 업데이트되면 검토자는 검토 상태에 따라 알림(AEM 및 이메일 모두)을 받습니다.

- **진행 중**: 작업을 계속 진행 중인 검토자에게 작업이 업데이트되었다는 알림이 표시됩니다.
- **완료됨**: 검토를 완료한 검토자는 재검토를 요청하는 알림을 받습니다.
