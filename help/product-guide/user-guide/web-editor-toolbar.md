...
메뉴 드롭다운에서는 아래와 같이 다양한 편집 작업에 액세스할 수 있습니다.

>[!BEGINTABS]

>[!TAB 새 편집기]

편집기 도구 모음의 ![메뉴 드롭다운](images/editor-toolbar-menu-dropdown-new-editor-2-0.png){width="350"}


>[!TAB 이전 편집기]

편집기 도구 모음의 ![메뉴 드롭다운](images/editor-toolbar-menu-dropdown-new.png){width="350"}
...
>[!NOTE]
>
> 새 편집기에서 **메뉴 표시**&#x200B;는 메뉴 드롭다운에서 더 이상 사용할 수 없습니다. 표시 메뉴에서 사용할 수 있는 다양한 옵션에 액세스하려면 [편집기 설정](./config-editor-settings.md)(으)로 이동합니다. 이전 편집기의 메뉴 드롭다운에서 표시 메뉴를 계속 사용할 수 있습니다.

이전 편집기의 표시 메뉴는 다음 기능으로 구성됩니다.
...
![블록 요소에 요소 삽입](images/note-in-para-insert-element_cs-editor-2-0.png)

>[!TAB 이전 편집기]

![블록 요소에 요소 삽입](images/note-in-para-insert-element_cs.png)
...
새 편집기에서 가로 커서가 나타날 때 기존 요소 사이에 새 요소를 삽입할 수 있습니다. 이 커서는 커서가 블록 요소 외부에 있으며 해당 위치에 새 요소를 추가할 수 있음을 나타냅니다.

![](images/Block-cursor-editor-2-0.png){width="300"}

예를 들어 DITA 항목에서 가로 커서가 `short description`과(와) `body` 사이에 나타나면 `prolog` 요소를 삽입할 수 있습니다. 그런 다음 `prolog`을(를) 사용하여 `copyright`, `author` 및 기타 세부 정보와 같은 메타데이터를 추가할 수 있습니다.
...
현재 또는 다음 유효한 위치에 단순 테이블 또는 테이블을 삽입합니다. 표 아이콘을 선택하여 **간단한 표 삽입** 대화 상자를 엽니다.
...
>[!NOTE]
>
> 향상된 테이블 편집 환경을 위해 새 편집기로 전환하도록 선택할 수도 있습니다. 새 편집기에서 사용할 수 있는 표 작성 기능에 대한 자세한 내용은 [새 편집기에서 표 작업](./web-editor-other-features.md#work-with-tables-in-the-new-editor)을 참조하세요.
...
**파일 선택** 대화 상자에서 **저장소** 또는 **컬렉션**&#x200B;에서 필요한 이미지 파일을 검색할 수 있습니다.
...
폴더를 쉽게 탐색하려면 상단의 이동 경로와 왼쪽의 폴더 탐색 패널을 사용하십시오.

![](images/select-file-browser-file-navigation.png)

파일 이름, 파일 제목 및 콘텐츠별로 필요한 이미지 파일을 검색할 수 있습니다. 또한 경로 \(검색할\), 문서 상태, 잠김, 마지막 수정 날짜 및 태그로 검색 결과를 필터링할 수도 있습니다. 파일 형식 필터는 **이미지**&#x200B;에 미리 적용되었으며 다른 파일 형식에 대해서는 유효한 파일 선택을 허용하도록 비활성화되어 있습니다. 이 경로 브라우저에서 필터를 사용하는 방법에 대한 자세한 내용은 [편집기의 기타 기능](./web-editor-other-features.md)을 참조하세요.

![](images/select-file-dialog-image-filter.png)

필요한 이미지 파일을 찾으면 파일을 선택하고 **선택**&#x200B;을 선택하여 문서에 이미지를 삽입합니다. `.png`, `.svg`, `.gif`, `.jpg`, `.eps`, `.ai`, `.psd` 등 다양한 형식의 이미지 파일을 삽입할 수 있습니다.

이미지 삽입 대화 상자에서 이미지/그림 제목과 이미지의 대체 텍스트를 추가할 수도 있습니다.
...
**Source 경로** 필드에 선택한 참조의 상대 경로가 표시됩니다. **경로 복사** 단추를 사용하여 절대 경로를 복사합니다.

**Source UUID** 필드에 선택한 참조의 UUID가 표시됩니다. **UUID 복사** 버튼을 사용하여 UUID를 복사합니다.
...
새 편집기에서 작성자 모드에서 핸들을 드래그하여 이미지 크기를 조정할 수도 있습니다.

- **모퉁이 핸들**: 모퉁이 핸들을 드래그하면 종횡비를 유지하면서 이미지 크기가 조정됩니다.

  ![](images/resize-corner-handles.png)

- **가운데 핸들**: **Shift**&#x200B;을 눌러 추가 가운데 핸들을 표시합니다. 이러한 구성 요소를 드래그하면 종횡비를 유지하지 않고 이미지 크기를 조정하므로 높이 또는 너비를 독립적으로 늘리거나 줄일 수 있습니다.

  ![](images/resize-middle-handles.png)

드래그하면 새 차원을 반영하도록 **콘텐츠 속성** 패널의 높이 및 너비 값이 자동으로 업데이트됩니다. 이 크기 조정은 **나란히** 모드 및 **미리 보기** 모드에도 반영되므로 게시 전에 크기 조정된 이미지가 어떻게 표시되는지 확인할 수 있습니다.
...
>[!NOTE]
>
>이미지에서 클릭 가능한 영역 \(image map\)을 정의할 수도 있습니다. 자세한 내용은 [왼쪽 패널](web-editor-left-panel.md) 섹션에서 **이미지 맵 삽입/편집** 기능 설명을 참조하십시오.
...
오디오 또는 비디오 파일 옵션을 선택하면 저장소 또는 컬렉션에서 원하는 파일을 찾아 선택할 수 있는 **파일 선택** 대화 상자가 표시됩니다.

![](images/select-file-dialog-multimedia.png){width="650"}

YouTube 또는 Vimeo를 선택하면 **멀티미디어 삽입** 대화 상자가 표시됩니다. 웹 링크 필드에 비디오 파일의 링크를 붙여넣고 삽입 을 선택하여 문서의 현재 또는 다음 유효한 위치에 비디오를 추가합니다.
...
>[!IMPORTANT]
>
> 참조된 콘텐츠의 상대 경로가 **링크 경로** 속성에 표시되더라도 내부적으로 참조된 콘텐츠의 UUID를 사용하여 링크가 만들어집니다.
...
**파일 선택** 대화 상자에서 **저장소**&#x200B;의 선택한 경로에 있는 파일에서 일부 텍스트를 검색할 수 있습니다. 예를 들어 아래 스크린샷에서는 *제동*&#x200B;이 검색됩니다.

>[!NOTE]
>
> 파일 및 경로 브라우저는 Experience Manager Guides as a Cloud Service 2601 릴리스에서 개선된 UI와 함께 도입되었습니다. 새 인터페이스는 기본적으로 활성화되어 있습니다. 이러한 업데이트 없이 기존 UI를 계속 사용하려면 고객 지원 팀에 문의하여 이 새로운 개선 사항을 비활성화하십시오.

![](images/reference-search-filters.png){width="650"}

상단의 이동 경로와 왼쪽의 폴더 탐색 패널을 사용하여 폴더를 쉽게 탐색할 수 있습니다.

![](images/select-file-browser-file-navigation.png)

파일 이름, 파일 제목 및 콘텐츠별로 필요한 파일을 검색할 수 있습니다. 오른쪽 패널에서 향상된 필터를 사용하여 검색 범위를 좁힐 수도 있습니다. 자세한 내용은 [편집기의 다른 기능](./web-editor-other-features.md)을 참조하세요.

![](images/select-file-dialog-image-filter.png)

검색된 텍스트가 포함된 필터링된 파일 목록이 표시됩니다. 파일 내용을 미리 볼 수도 있습니다.
...
키워드를 마우스 오른쪽 단추로 클릭하고 속성 옵션을 선택하여 삽입된 키워드의 속성을 변경할 수도 있습니다. **키워드 특성** 대화 상자가 열립니다.
...
버전 정보에는 주제 또는 맵의 현재 버전이 표시됩니다. 버전 번호 옆에 별표 `(*)`이(가) 표시되어 변경 내용이 문서에 저장되었지만 버전이 지정되지 않았음을 나타냅니다.
