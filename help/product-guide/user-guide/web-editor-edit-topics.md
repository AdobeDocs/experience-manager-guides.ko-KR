...
1. 항목에서 요소를 편집하거나 삽입하려면 필수 요소의 텍스트 경계 내부를 클릭하여 변경하거나, 새 요소를 추가할 요소의 끝에 커서를 놓고 도구 모음에서 필수 요소를 선택합니다(또는 Alt+1을 눌러 요소 삽입 팝업 열기). 이 팝업은 항목의 해당 위치에 대해 유효한 요소만 지능적으로 나열하고 삽입합니다.

1. 또한 빠른 삽입 메뉴를 사용하여 허용된 요소를 커서 위치에 쉽게 삽입할 수 있습니다. 요소에 액세스하려면 Windows의 경우 **Control + /**, Mac의 경우 **Command + /**&#x200B;을(를) 선택하십시오.

   ![빠른 삽입 메뉴](./images/quick-insert-menu-in-editor.png){width="650"}

   새 요소를 검색하거나 빠른 삽입 메뉴를 사용하여 즐겨찾기에서 요소를 선택한 다음 현재 커서 위치에 삽입합니다. 즐겨찾기에는 가장 자주 사용하는 요소가 포함되며 현재 커서 위치에 적합한 요소만 표시됩니다. [편집기 설정](./config-editor-settings.md)에서 사용할 수 있는 빠른 삽입 메뉴를 통해 이 기능을 활성화하거나 비활성화하고 삽입하기 좋아하는 요소를 구성할 수 있습니다.
...
- 형식 지정 : 선택한 콘텐츠의 형식을 지정하는 것은 아래 그림과 같이 편집기 1.0에 비해 새 편집기에서 훨씬 더 쉽습니다.

>[!BEGINTABS]

>[!TAB 새 편집기]

상황별 도구 모음을 사용하여 선택한 컨텐츠의 서식을 굵게, 기울임꼴 또는 밑줄로 지정할 수 있습니다. 콘텐츠를 선택한 다음 나타나는 메뉴에서 적절한 서식 아이콘을 클릭합니다. 선택한 컨텐츠를 굵게, 기울임체 또는 밑줄로 만듭니다. 그런 다음 유효한 열린 태그의 콘텐츠가 병합되어 단일 요소 아래에 나타납니다.

![서식 옵션](./images/formatting-options.png){width="650"}

>[!TAB 이전 편집기]

선택한 컨텐츠를 굵은 기울임꼴로 만들고 선택한 컨텐츠에 밑줄을 긋습니다. 그런 다음 유효한 열린 태그의 콘텐츠가 병합되어 단일 요소 아래에 나타납니다. 예를 들어 단락 내의 콘텐츠를 선택하고 선택 영역을 다른 단락으로 확장할 수 있습니다. 그런 다음 선택한 컨텐츠를 굵게 만들면 열려 있는 태그의 모든 굵은 컨텐츠가 병합되어 단일 단락 요소 아래에 나타납니다.

>[!ENDTABS]

- 삭제: 선택한 컨텐츠를 삭제하면 열려 있는 태그에서 삭제 후 남은 컨텐츠가 병합됩니다.
...
![작업 복사 표시기](images/working-copy-text-update-indicator-new-editor-2-0.png){width="550"}

>[!TAB 이전 편집기]

이 보기는 콘텐츠가 이전 편집기에서 렌더링되는 방법을 표시합니다.

![작업 복사 표시기](images/working-copy-text-update-indicator.png){width="550"}
...
>[!NOTE]
>
> [파일 속성](./web-editor-right-panel.md#file-properties)에서 사용하거나 백엔드에서 적용한 메타데이터 필드를 변경하면 문서 버전에서 별표 `(*)`도 트리거됩니다.  시스템에서 생성한 메타데이터 업데이트가 이 지표에 영향을 주지 않도록 하기 위해 관리자는 메타데이터 속성에 대한 무시 목록을 구성할 수 있습니다. 메타데이터 속성을 구성하는 방법에 대한 자세한 내용은 [메타데이터 속성 무시 목록 구성](../install-conf-guide/conf-metadata-prop.md)을 참조하십시오.

>[!BEGINTABS]

>[!TAB 새 편집기]

![버전 업데이트 표시기](images/version-update-indicator-editor-2-0.png){width="650"}

>[!TAB 이전 편집기]

![버전 업데이트 표시기](images/version-update-indicator.png){width="650"}
