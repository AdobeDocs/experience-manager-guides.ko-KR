...
1. 사용자는 폴더 프로필에 구성된 언어 그룹을 볼 수 있습니다. 언어 그룹은 언어 코드와 함께 언어 폴더를 표시합니다. 예를 들어 G1이라는 언어 그룹에는 이탈리아어 \(it\), 독일어 \(de\), 프랑스어 \(fr\) 및 영어 \(en\) 언어 폴더가 포함되어 있습니다.
   ...
   ![번역 패널](images/translation-languages.png){width="300"}
...
1. **적용**&#x200B;을 선택합니다. 주제 및 관련 에셋의 세부 정보가 포함된 목록이 표시됩니다.
   ...
   >[!NOTE]
   >
   > DITAVAL 및 Markdown 파일 참조를 사용하여 맵을 번역할 때, 그리고 번역이 작업 사본을 기반으로 하는 경우, 소스 언어 폴더에 이미지 및 기타 연결된 에셋과 같은 참조가 존재하면 이러한 참조가 포함됩니다. 이러한 참조는 번역 대시보드의 참조 목록에 나타나며, 이를 사용하여 번역용으로 명시적으로 선택할 수 있습니다. 번역하는 동안 참조된 에셋은 타겟 언어 폴더로 복사되고 표준 이미지 번역 동작으로 일관되게 처리됩니다.
...
1. 번역을 위해 전송할 주제를 선택합니다. 다음 열에 대해 항목 필터링 옵션을 사용할 수도 있습니다.
   ...
   - **버전**: 원본 파일의 버전 번호입니다.
     ...
     파일에 아직 버전이 지정되지 않은 변경 사항(즉, 맵에서 새 버전으로 저장되지 않은 변경 사항)이 저장된 경우 파일 옆에 버전이 지정되지 않은 변경 사항이 있음을 나타내는 정보 아이콘이 표시됩니다.

     ![](images/unversioned-changes-translation.png){width="650"}

     >[!NOTE]
     >
     > 버전이 없는 변경 내용이 있는 파일만 보려면 [필터] 패널에서 **버전이 없는 변경 내용만 있는 에셋 표시** 설정을 활성화하십시오. 또한 버전 없는 표시기는 최신 버전을 기반으로 파일을 번역할 때만 표시됩니다.
...
1. **경로** 필드에서 새 번역 프로젝트를 만들 위치를 선택합니다.

   - 새 번역 프로젝트를 만드는 경우 프로젝트를 만들 폴더를 선택합니다.

     ![프로젝트 경로 입력](images/project-path.png){width="650"}
...
1. **강제 동기화**&#x200B;를 선택하십시오. 선택한 동기화되지 않은 에셋에 대해 [동기화 중]으로 상태를 설정합니다.
