...
- [소스](#source-view)
- [미리보기](#preview)
- [나란히 보기](#side-by-side-view-available-in-new-editor-only)
...
파일 선택 대화 상자를 표시합니다. 삽입할 주제/맵 파일로 이동한 다음 **선택**&#x200B;을 선택하여 맵에 추가합니다.
...
>[!NOTE]
>
>참조를 드래그 앤 드롭하여 맵에서 이동할 수도 있습니다. 드래그 앤 드롭 후 참조를 처리하는 방법은 사용 중인 편집기에 따라 다릅니다. 편집기 2.0에서 참조(예: 주제, 하위 맵 또는 키 정의)를 맵의 기존 참조에 드래그하여 놓으면 기존 참조가 대체되지 않고 자식 요소로 추가됩니다. 이전 편집기에서 동일한 작업이 참조의 URL을 하위 요소로 추가하는 대신 업데이트합니다.
...
**작성자** 보기를 사용하면 편집기에서 DITA 맵을 편집할 수 있습니다. 맵 편집기의 WYSIWYG 보기를 표시합니다.
...
**메뉴** 드롭다운에서 사용할 수 있는 옵션은 [편집기 도구 모음](./web-editor-toolbar.md#menu-dropdown)에서 사용할 수 있는 옵션과 동일합니다.

또한 다음 아이콘을 보고 작성자 보기에서 관련 작업을 수행할 수 있습니다.

**다음 항목 앞에 삽입/다음 항목 뒤에 삽입** - ![](images/insert_element_before_icon.svg) / ![](images/insert_element_after_icon.svg)

**앞에 요소 삽입** 또는 **뒤에 요소 삽입** 대화 상자를 표시합니다. 맵에 삽입할 요소를 선택합니다. 작업에 따라 새 요소는 맵에서 현재 요소의 앞 또는 뒤에 삽입됩니다.
...
>[!NOTE]
>
> 새 편집기에서 편집기 설정에서 **태그**&#x200B;를 추가하거나 제거하는 옵션을 사용할 수 있습니다. 자세한 내용은 [편집기 설정](./config-editor-settings.md)을 참조하세요.
...
>[!NOTE]
>
> 새 편집기에서 **변경 내용 추적** 기능은 현재 지원되지 않습니다.
...
>[!NOTE]
>
> 이 섹션은 이전 편집기에만 적용됩니다. 새 편집기에서 맵과 해당 주제가 자동으로 새로 고쳐집니다.

미리보기 모드에서 맵을 열고 편집기의 다른 탭에서 다시 맵으로 전환하면 미리보기가 자동으로 새로 고쳐지지 않습니다. 맵 미리 보기를 다시 로드하려면 수동으로 새로 고쳐야 합니다.

맵의 미리보기 모드에서 사용할 수 있는 새로 고침 기능은 다음 두 가지 방법으로 사용할 수 있습니다.
...
## 나란히 보기(새 편집기에서만 사용 가능)

새 편집기의 나란히 보기를 사용하면 작성자 및 Source 보기를 나란히 볼 수 있습니다. 이렇게 하면 맵의 작성 방법을 보다 쉽게 이해할 수 있으며, 변경한 내용이 의도한 구조와 일치하도록 합니다.

예를 들어 항목이 출력에 올바르게 표시되지 않는 경우 작성자 보기를 사용하여 맵에서 해당 배치를 확인하는 동시에 Source 보기를 검토하여 항목 참조 또는 속성을 검토할 수 있습니다.
...
1. 탐색기로 이동하여 편집할 맵 파일을 엽니다.

   맵 파일이 맵 보기에서 열립니다.

   >[!NOTE]
   >
   > Assets UI를 사용하여 맵 파일을 열 수도 있습니다. 편집할 항목이 포함된 DITA 맵 파일로 이동하고 기본 도구 모음에서 **항목 편집**&#x200B;을 선택하여 편집기를 시작합니다.

1. 편집할 항목을 편집기에서 선택합니다.
...
오른쪽 패널에는 맵 편집기의 레이아웃 보기에 콘텐츠 속성 및 맵 속성이 표시됩니다.

**콘텐츠 속성**

콘텐츠 속성 패널에는 해당 유형, 링크 경로, 링크 UUID 및 속성을 포함하여 맵에서 현재 선택한 주제에 대한 정보가 포함되어 있습니다.

자세한 내용은 편집기에서 [콘텐츠 속성](web-editor-features.md#right-panel)을 참조하세요.
...
![레이아웃 보기](images/layout-inline-attributes.png){width="650"}

새 편집기 및 이전 편집기의 작성자 보기에 특성이 표시되는 방식을 비교하려면 아래 탭을 참조하십시오.

>[!BEGINTABS]

>[!TAB 새 편집기]

![작성자 보기](images/author-inline-attributes-editor-2-0.png){width="650"}

>[!TAB 이전 편집기]

![작성자 보기](images/author-inline-attributes.png){width="650"}
