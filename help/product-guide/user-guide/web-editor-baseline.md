...
>[!NOTE]
>
> Experience Manager Guides 2026.03.0 릴리스에서는 향상된 성능과 안정성을 제공하는 새로운 기준선(Beta)을 사용할 수 있습니다. 이 기준선을 사용하려면 고객 지원 팀에 문의하여 기능을 활성화하십시오. [새 기준선(Beta)](./web-editor-baseline-v2.md)에 대해 자세히 알아보세요.
...
1. **새 기준선** 대화 상자에서 다음 세부 정보를 제공합니다.
   ...
   ![기준선 패널](images/baseline-manage.png){width="500"}
...
![기준선 만들기](images/dynamic-baseline.png){width="300"}
...
- **레이블**: 주제에 지정된 레이블이 있는 경우 **레이블** 드롭다운을 사용하여 [나열된 레이블](#labels-list) 중에서 선택하십시오.
  ...
  동적 기준선의 경우, 맵의 저장된 최신 버전 및 현재 작업 카피에서 레이블을 가져옵니다. 예를 들어 항목 A의 버전 1.0 및 1.1에 대해 레이블 `Label Release A.1.0 ` 및 `Label Release A.1.1`을(를) 만들고 항목 B의 버전 1.0 및 1.1에 대해 레이블 `Label Release B.1.0` 및 `Label Release B.1.1`을(를) 만든 경우. 그런 다음 항목 A를 추가하여 버전 1.0에서 A를 매핑하고 항목 B를 추가하여 1.0*에서 A를 매핑할 수 있습니다(작업 복사본). 이 경우 동적 기준선 레이블의 드롭다운에서 `Label Release A.1.0 `, `Label Release A.1.1`, `Label Release B.1.0` 및 `Label Release B.1.1`을(를) 볼 수 있습니다.
...
![기준선을 복제합니다](images/baseline-duplicate.png){width="300"}
...
![기준 필터](images/baseline-filter.png){width="300"}
