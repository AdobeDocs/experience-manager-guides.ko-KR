...
1. 메타데이터를 업데이트하려면 업데이트할 파일을 선택합니다. **Title** 옆에 나타나는 확인란을 선택하여 [메타데이터] 패널에 나타나는 파일을 선택할 수 있습니다.

   모든 레코드를 선택하려면 제목 표시줄 위의 **모두 선택** 탭을 사용하십시오. 스크롤할 때 레코드가 점진적으로 로드되므로 **모두 선택**&#x200B;을 사용하기 전에 목록 끝으로 스크롤해야 합니다. 이렇게 하면 현재 표시된 레코드뿐만 아니라 모든 파일에 대해 편집 권한 및 잠금 상태를 평가할 수 있습니다.

   >[!NOTE]
   >
   > 다른 사용자가 파일을 잠근 경우 해당 파일을 선택하여 메타데이터를 업데이트할 수 없습니다. 또한 잠금 아이콘은 잠긴 파일의 제목 근처에 나타납니다. 아이콘 위로 마우스를 가져가 사용자의 이름을 볼 수 있습니다. 선택한 파일이 읽기 전용 모드에 있는 경우 **관리** 단추를 구성하여 사용하지 않도록 설정할 수 있습니다. 자세한 내용은 메타데이터 패널에서 [관리 단추](../guides-ui-extensions/examples.md)를 참조하십시오.
   >
   > <br>이 제한은 DITA 및 Markdown 파일에만 적용됩니다. DITA가 아닌 에셋(예: 이미지 및 멀티미디어)의 경우 메타데이터 속성은 읽기 전용 모드에서도 편집할 수 있습니다.
...
1. 사용자 지정 메타데이터 열을 추가하려면 메타데이터 페이지의 오른쪽 위 모서리에 있는 세 개의 점에서 **Workspace 설정**(버전 5.2 **이전의** On-Premise 설정에 대해 **설정**(으)로 표시)을 선택합니다.
...
1. **파일 형식** 및 **링크 형식**&#x200B;을(를) 기반으로 끊어진 링크를 필터링할 수 있습니다. 드롭다운에서 선택한 항목에 따라 끊어진 링크 목록이 표시됩니다. 예를 들어 DITA 맵의 컨텐트 참조만 표시하도록 선택할 수 있으며, 파일에 사용된 깨진 컨텐트 참조만 표시됩니다.
   ...
   DITA 항목, DITA 맵, 파일 참조, 키 참조, 콘텐츠 참조, 콘텐츠 키 참조, 이미지 참조, 멀티미디어 파일 참조 및 멀티미디어 키 참조는 **링크 유형** 드롭다운에 사용할 수 있는 값이고, 항목, 맵, 문서, 이미지, 비디오, 오디오 및 기타 값은 **파일 유형** 드롭다운에 사용할 수 있습니다.
1. 다음 필터링 옵션을 사용하여 목록에 다음 열을 표시하도록 선택할 수도 있습니다.

   - **끊어진 링크**(기본적으로 선택됨) 끊어진 링크의 경로가 DITA 맵에 지정됩니다.

   - **링크 유형**(기본적으로 선택됨) 링크 유형입니다. 사용 가능한 옵션은 항목, 맵, 파일 참조, 키 참조, 컨텐츠 참조, 컨텐츠 키 참조, 이미지 참조, 멀티미디어 파일 참조 및 멀티미디어 키 참조입니다.

   - **다음에서 사용**(기본적으로 선택됨) 끊어진 링크가 사용된 참조입니다. 참조를 선택하여 작성자 모드로 볼 수 있습니다.

   - **파일 형식**(기본적으로 선택됨) 참조 형식(항목, 맵, 문서, 이미지, 비디오, 오디오 및 기타)입니다.
