...
참조된 콘텐츠의 경우 패널에는 선택한 참조를 식별하고 복사하는 데 도움이 되는 **링크 경로** 및 **링크 UUID** 옵션도 표시됩니다.

>[!NOTE]
>
> HTML 기반 파일의 경우 링크 경로 및 링크 UUID 옵션을 사용할 수 없습니다. 이러한 파일은 기존 **링크 URL** 동작을 계속 사용합니다.

![](images/web-editor-cross-ref-options.png)

**유형**: 드롭다운에서 현재 태그에 대한 전체 계층 구조의 태그를 보고 선택합니다.

**링크 경로**: 선택한 참조의 상대 경로를 표시합니다. **경로 복사**&#x200B;를 사용하여 절대 경로를 복사합니다.

**링크 UUID**: 선택한 참조의 UUID를 표시합니다. **UUID 복사**&#x200B;를 사용하여 UUID를 복사합니다.

유효한 UUID를 링크 경로 필드에 직접 붙여 넣으면 자동으로 절대 파일 경로로 확인되고 해당 UUID가 링크 UUID 필드에 표시됩니다. 이렇게 하면 에셋 경로와 해당 UUID 기반 참조를 보다 쉽게 식별하고 복사할 수 있습니다.
...
>[!NOTE]
>
> [파일 속성] 패널은 파일과 관련된 다양한 메타데이터 속성을 보고 수정하는 옵션을 제공합니다. 그러나 파일이 읽기 전용 모드인 경우 이러한 메타데이터 속성을 수정할 수 없습니다. 이 제한은 DITA 및 Markdown 파일에만 적용됩니다. DITA가 아닌 에셋(예: 이미지 및 멀티미디어)의 경우 메타데이터 속성은 읽기 전용 모드에서도 편집할 수 있습니다.
...
- **추가 속성을 편집**: 현재 열려 있는 파일의 추가 속성을 보고 편집할 수 있습니다.
...
파일을 여는 것 외에도 참조 섹션의 **옵션** 메뉴를 사용하여 여러 작업을 수행할 수 있습니다. 수행할 수 있는 작업에는 편집, 미리보기, UUID 복사, 경로 복사, 컬렉션에 추가 및 속성이 포함됩니다.

**번역**

이 섹션에는 편집기에서 현재 열려 있는 에셋에 사용할 수 있는 모든 언어 사본이 알파벳순으로 나열됩니다. 정보는 표 형식으로 표시되며 해당 *파일 제목*(또는 *파일 제목*&#x200B;을 사용할 수 없는 경우 *파일 이름*)과 함께 각 언어 코드를 표시합니다.
