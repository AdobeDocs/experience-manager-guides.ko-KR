...
>[!TIP]
>
> 기준선 작업에 대한 모범 사례는 모범 사례 안내서에서 *기준선* 섹션을 참조하십시오.

관리자는 맵 대시보드에서 기준선 탭을 구성할 수 있습니다. 자세한 내용은 설치 및 구성 안내서의 DITA 맵 대시보드에서 *기준선 탭 구성* 섹션을 참조하십시오.

**기준선 탭**&#x200B;에서 다음 작업을 수행할 수 있습니다.

- [기준선 만들기](#create-a-baseline)
- [베이스라인의 내용 보기](#view-contents-of-a-baseline)
- [기준선 편집, 복제 또는 제거](#edit-duplicate-or-remove-baselines)
- [기준선에 레이블 추가](#add-labels-to-a-baseline)
...
1. Assets UI에서 DITA 맵 파일을 열고 **기준선** 페이지로 이동합니다.
2. 왼쪽 상단에서 **만들기**&#x200B;를 선택합니다.
3. [기준 요소] 페이지의 **기준 요소 이름** 필드에 기준 요소 이름을 입력합니다.

   ![](images/create-baseline-assets-ui.png){width="300" align="left"}

4. **다음을 기준으로 버전 설정**&#x200B;에서 다음 옵션 중 하나를 선택합니다.
   ...
   - **레이블**: 이 옵션을 선택하면 적용된 레이블에 따라 주제를 선택할 수 있습니다. 입력한 문자열을 기반으로 목록을 필터링하려면 레이블을 입력합니다. 필터링된 목록에서 레이블을 선택하여 지정된 레이블이 있는 주제 및 기타 에셋을 선택할 수 있습니다.
     ...
     레이블 을 선택하면 지정된 레이블이 적용되지 않은 최신 버전의 주제를 사용할 수 있는 추가 옵션도 제공됩니다. 이 옵션을 선택하지 않고 지정된 레이블이 없는 주제 또는 미디어 파일이 있는 경우 기준선 작성 프로세스가 실패합니다. 레이블 추가에 대한 자세한 내용은 레이블 사용을 참조하십시오.

   - **Version on**: 지정한 날짜 및 시간의 항목 버전을 선택합니다. 여기서 지정하는 시간은 Adobe Experience Manager 서버의 시간대에 해당합니다. 서버가 다른 시간대에 있는 경우 항목은 로컬 시간대가 아니라 서버의 시간대에 따라 선택됩니다.
...
베이스라인 탭을 선택하고 목록에서 원하는 베이스라인 버전을 선택하면 기존 베이스라인의 내용을 볼 수 있습니다. [기준선] 페이지는 DITA 맵 파일, 맵의 내용 또는 주제 및 참조된 내용의 세 부분으로 나뉩니다. 맵에 하위 맵이 포함된 경우 하위 맵에서 참조된 항목이 컨텐트 섹션에도 표시됩니다. 베이스라인 페이지의 다양한 열이 아래에 설명되어 있습니다.
...
1. DITA 맵에서 참조하는 모든 주제 및 미디어 파일을 로드하려면 **모든 주제 찾아보기** 링크를 선택합니다. 항목 및 미디어 파일의 UUID는 항목 제목 또는 \(media\) 파일 이름 아래에도 표시됩니다.

   >[!NOTE]
   >
   > DITA 맵에 중첩된 맵과 주제가 있는 매우 큰 파일 세트가 있는 경우 모든 주제 찾아보기를 선택하면 모든 파일을 로드하는 데 시간이 걸릴 수 있습니다.
...
기준선을 선택하고 **복제**&#x200B;를 선택하여 기존 기준선의 복사본을 만듭니다. 기준선에 다른 이름을 지정하고 주제 및 참조된 콘텐츠의 버전 번호를 선택한 다음 **저장**&#x200B;을 선택하십시오.

**기준선 제거**

기준선 버전을 선택하고 **제거**&#x200B;를 선택하여 기준선을 제거하십시오.

## 기준선에 레이블 추가

모든 단일 주제에 레이블을 추가하는 것은 시간이 많이 소요될 수 있습니다. Experience Manager Guides은 DITA 맵에서 여러 주제 및 참조된 콘텐츠에 레이블을 추가하는 한 번의 클릭 메커니즘을 제공합니다.
