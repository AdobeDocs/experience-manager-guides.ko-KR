...
| **멀티미디어 파일 포함** | 오디오 및 비디오와 같은 지원되는 멀티미디어 자산을 생성된 PDF에 직접 포함하려면 이 옵션을 선택합니다. 활성화되면 비디오 컨텐츠는 PDF에서 포스터 이미지 또는 썸네일을 사용하여 표시되며, 포함된 멀티미디어를 지원하는 호환되는 PDF 뷰어에서 재생할 수 있습니다. 비디오 재생의 경우 최신 버전의 Adobe Acrobat과 같이 지원되는 애플리케이션에서 PDF을 열고 메시지가 표시되면 문서를 신뢰합니다. 대부분의 브라우저 기반 PDF 뷰어는 포함된 멀티미디어 재생을 지원하지 않습니다. |
