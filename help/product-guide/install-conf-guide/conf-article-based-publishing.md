...
Experience Manager Guides 설정을 기반으로 문서 기반 출력을 게시하기 위한 AEM 사이트를 만들기 위한 지침을 제공하는 탭은 Cloud Service 또는 온프레미스입니다.

>[!BEGINTABS]

>[!TAB Cloud Service]

1. [XML Documentation 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html)에서 **Cloud Service용 Adobe 구성 요소 콘텐츠 패키지**&#x200B;를 다운로드하십시오.
1. AEM 패키지 관리자를 엽니다. 패키지 관리자에 액세스할 기본 URL은 `https://<hostname>/crx/packmgr/index.jsp`입니다.
1. Cloud Service용 XML Documentation 구성 요소 콘텐츠 패키지를 업로드한 다음 설치합니다.
...
AEM 사이트를 사용하여 편집기의 출력 사전 설정을 사용하여 문서를 게시할 수 있습니다.

>[!TAB 온-프레미스]

문서 기반 게시를 활성화하려면 Adobe 소프트웨어 배포 포털에서 다음 패키지를 다운로드하여 설치하십시오. 이를 설치하여 Tragopan 사이트를 만듭니다. \(Tragopan 사이트는 카테고리, 섹션 및 문서 페이지에 대한 템플릿이 있는 샘플 기술 자료 AEM 사이트입니다.\) 편집기의 출력 사전 설정을 사용하여 AEM 사이트 출력을 생성하도록 Tragopan 사이트를 업데이트합니다.
