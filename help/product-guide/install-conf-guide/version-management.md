...
Experience Manager Guides 설정에 따라 **업로드된 파일에 대한 새 버전 만들기** 옵션을 사용하도록 설정하는 지침은 Cloud Service 또는 온프레미스입니다.
...
AEM Guides 편집기를 사용하여 DITA 주제를 만들고 업데이트할 수 있습니다. 저장소에서 체크 아웃된 문서만 편집할 수 있도록 편집기를 구성할 수 있습니다. 이렇게 하면 다른 작성자가 편집을 위해 연 주제를 다른 작성자가 실수로 덮어쓰지 않습니다. 편집을 위해 주제를 열면 작성자는 파일을 닫을 때 파일을 체크 인할 수 있습니다.

또 다른 중요한 규칙은 체크 아웃된 파일이 시스템에 다시 체크 인되도록 하는 것입니다. 따라서 사용자가 파일을 다시 체크 인하지 않고 실수로 닫지 않게 됩니다.

Experience Manager Guides 설정을 기반으로 체크 아웃된 파일의 편집을 허용하는 설정을 구성하는 지침은 Cloud Service 또는 온프레미스에서 제공됩니다.

>[!BEGINTABS]

>[!TAB Cloud Service]

구성 파일을 만들려면 [구성 재정의](download-install-config-override.md#)의 지침을 사용하십시오. 구성 파일에서 다음 \(property\) 세부 정보를 입력하여 체크 아웃된 파일의 편집을 구성합니다.
...
다음 탭은 사용자가 Experience Manager Guides 설정을 기반으로 다른 사용자 또는 다른 사용자가 체크 아웃한 파일을 업로드할 때 덮어쓸 수 있도록 허용하는 지침을 제공합니다. Cloud Service 또는 온-프레미스.
...
| PID | 속성 키 | 속성 값 |
|---|------------|--------------|
| `com.adobe.fmdita.config.ConfigManager` | `overwrite.checkout.onupload` | 부울 \(true/false\).<br> **기본값**: `false` |
...
다음 탭은 사용자가 Experience Manager Guides 설정에 따라 체크 아웃된 파일 또는 다른 사용자가 실수로 삭제하지 못하도록 하는 지침을 제공합니다. Cloud Service 또는 온-프레미스.

>[!BEGINTABS]

>[!TAB Cloud Service]

구성 파일을 만들려면 [구성 재정의](download-install-config-override.md#)의 지침을 사용하십시오. 사용자가 체크 아웃된 파일을 실수로 삭제하지 않도록 구성 파일에서 다음 \(property\) 세부 정보를 제공합니다.
...
새 인덱스 속성 외에 `/oak:index/damAssetLucene`에 다음 속성이 설정되어 있는지 확인하십시오.
...
Experience Manager Guides 설정을 기반으로 다른 파일에서 참조하거나 참조를 포함하는 파일을 삭제할 수 있는 사용자를 정의하는 지침은 Cloud Service 또는 온프레미스 탭을 제공합니다.

>[!BEGINTABS]

>[!TAB Cloud Service]

구성 파일을 만들려면 [구성 재정의](download-install-config-override.md#)의 지침을 사용하십시오. 구성 파일에서 다음 \(property\) 세부 정보를 제공하여 참조가 포함되어 있거나 다른 파일에서 참조하는 파일을 삭제할 수 있는 사용자를 정의합니다.

| PID | 속성 키 | 속성 값 |
|---|------------|--------------|
| `com.adobe.fmdita.config.ConfigManager` | `block.unsafe.delete` | 가능한 값은 <br> - allow\_unsafe\_delete\_for\_all <br> - allow\_unsafe\_delete\_for\_delete\_assets\_group <br> - block\_unsafe\_delete\_for\_all <br>입니다. **기본값**: `allow_unsafe_delete_for_delete_assets_group` <br> 이러한 상수에 대한 자세한 내용은 아래에 나와 있습니다. |
...
콘텐츠를 업데이트하고 새 버전을 만들면 저장소의 이전 파일 버전이 유지됩니다. 일정 기간 동안 파일에 대해 여러 버전이 작성될 수 있으며, 일괄적으로 리포지토리에 많은 공간을 사용할 수 있습니다. AEM Guides을 사용하면 저장소에서 삭제해야 하는 이전 버전을 구성할 수 있습니다.

>[!NOTE]
>
> DITA 및 비 DITA 파일을 제거할 수 있습니다.

지정된 기준을 충족하는 파일의 버전은 유지 관리되며 삭제되지 않습니다.
...
1. **제거 취소** 또는 **제거 시작**&#x200B;을 선택할 수 있습니다.
   ...
   삭제된 버전을 보려면 **버전 제거 보고서 다운로드**&#x200B;를 선택하십시오. 이 보고서는 모든 버전의 삭제 상태와 함께 특정 버전이 유지된 이유 또는 삭제된 이유를 제공합니다.
