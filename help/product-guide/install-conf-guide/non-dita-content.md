...
### 기존 Word 문서를 DITA 주제 유형 문서로 변환

Experience Manager Guides 설정을 기반으로 기존 Word 문서를 DITA 주제 유형 문서로 변환하는 방법은 Cloud Service 또는 On-Premise 탭을 참조하십시오.
...
1. 소스 Word 문서를 이 폴더에 업로드합니다. DAM의 콘텐츠 업로드에 대한 자세한 내용은 [기존 DITA 콘텐츠 업로드](upload-dita-content.md#)를 참조하십시오.
...
변환 프로세스를 진행하려면 [변환을 위해 InDesign 파일을 준비](aemg-appendix.md#id195DBF0045Z)하고 [InDesign에서 DITA로의 마이그레이션을 위해 매핑 파일을 준비](aemg-appendix.md#id194AF0003HT)해야 합니다. 그런 다음 변환 프로세스를 실행하는 지정된 절차를 따라야 합니다.
...
1. `config` 노드 내에 `apps` 폴더의 오버레이 노드를 만듭니다.
   ...
   - `mapStyle` 요소에서 InDesign 문서 스타일과 DITA 요소의 매핑이 포함된 맵 파일의 위치를 지정합니다. 기본 매핑은 다음 위치에 있는 파일에 저장됩니다.
     ...
     >[!NOTE]
     >
     > `stmap.adobeidml.xml` 파일의 구조와 사용자 지정 방법에 대한 자세한 내용은 [부록](aemg-appendix.md#id194AF0003HT)을 참조하십시오.
...
1. 소스 InDesign 문서를 이 폴더에 업로드합니다. DAM의 콘텐츠 업로드에 대한 자세한 내용은 [기존 DITA 콘텐츠 업로드](upload-dita-content.md#)를 참조하십시오.
...
### 기존 XHTML 문서를 DITA 주제로 변환

Experience Manager Guides 설정을 기반으로 기존 XHTML 문서를 DITA 주제로 변환하는 방법은 Cloud Service 또는 On-Premise 탭을 참조하십시오.
...
1. 소스 XHTML 문서를 이 폴더에 업로드합니다. DAM의 콘텐츠 업로드에 대한 자세한 내용은 [기존 DITA 콘텐츠 업로드](upload-dita-content.md#)를 참조하십시오.
...
구조화되지 않은 Adobe FrameMaker 컨텐츠(.fm 및 .book)를 구조화된 DITA로 변환하려면 FrameMaker의 변환 테이블 메커니즘을 사용할 수 있습니다. 이 프로세스는 템플릿 기반 접근 방식을 사용하여 기존 콘텐츠를 평가하고 변환 테이블을 통해 FrameMaker 스타일을 DITA에 매핑하는 데 중점을 둡니다. 자세한 내용은 [Adobe FrameMaker의 비정형에서 DITA로 기술 문서 마이그레이션](https://migrate-from-unstructured-to-dita-step-by-step-guide.meetus.adobeevents.com/)을 참조하십시오.

변환 후 구조화된 콘텐츠를 AEM Guides으로 마이그레이션할 수 있습니다. 자세한 내용은 [WebDAV 도구와 FrameMaker을 사용하여 기존 DITA 콘텐츠를 업로드](./upload-dita-content.md)를 참조하십시오.
...
Experience Manager Guides 설정을 기반으로 다른 구조화된 문서를 마이그레이션하기 위한 지침을 제공하는 탭은 Cloud Service 또는 온프레미스입니다.
...
1. 소스 구조화된 문서를 이 폴더에 업로드합니다. DAM의 콘텐츠 업로드에 대한 자세한 내용은 [기존 DITA 콘텐츠 업로드](upload-dita-content.md#)를 참조하십시오.
