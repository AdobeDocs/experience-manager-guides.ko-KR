...
AEM Guides을 사용하면 폴더 수준뿐만 아니라 엔터프라이즈 \(전역\) 수준에서 작성 \(topic 또는 map\) 템플릿, 출력 템플릿, 조건부 속성 및 편집기 구성을 구성할 수 있습니다. 이렇게 하면 기업의 다양한 부서나 제품에 대한 구성을 분리할 수 있습니다.
...
1. 조건부 특성에 대해 **이름**, **값** 및 **레이블**&#x200B;을(를) 입력하십시오.
   ...
   속성 이름만으로 프로파일을 저장할 수 있습니다. 그러나 속성에 값이 지정된 경우에만 속성을 사용할 수 있습니다. 속성에 대해 - value와 label을 모두 지정하면 조건부 속성의 레이블이 편집기에 표시됩니다. 또한 조건부 사전 설정을 만들 때 게시 관리자에게 레이블이 표시됩니다.
...
사용자 지정 속성을 사용하는 경우 DTD에서 지원하는 유효한 DITA 속성이어야 합니다. Experience Manager Guides 설정을 기반으로 하는 표준 DITA 속성이 아닌 Cloud Service 또는 온프레미스 탭을 사용하여 속성을 사용하는 방법에 대한 지침을 제공합니다.
...
1. 사용자 지정 속성을 DTD 파일에 추가합니다. 예를 들어, DTD 파일이 commonElements.mod이면 이 파일은 DTD 디렉토리에서 찾아야 합니다. 시스템 DTD 파일의 기본 경로는
   ...
   >[!IMPORTANT]
   >
   > 특수화된 DTD 파일은 사용자 지정 코드 배포의 일부여야 합니다. /apps 아래의 DTD는 제품 배포의 일부이므로 새 릴리스의 설치로 덮어쓰기됩니다. 프로젝트 폴더 내의 /var/dxml/dita\_resources 아래에 특수화된 DTD를 추가하고 DITA 프로파일에 DTD/카탈로그 경로를 포함하는 것이 좋습니다.자세한 내용은 [DITA 특수화 통합](dita-ot-specialization.md#id211MB0E00XA)을 참조하십시오.

1. 패키지 관리자를 사용하여 `/libs/fmdita/config/condAttrList.csv` 파일을 다운로드합니다. 사용 중인 버전에 따라 `libs/fmdita/config/condAttrList.xml` 파일도 다운로드할 수 있습니다.

1. Cloud Manager의 Git 저장소의 다음 위치에 `condAttrList.csv` 파일의 복사본을 만듭니다.
...
1. 다음 위치에서 사용할 수 있는 `condAttrList.csv` 파일로 이동합니다.

   `/libs/fmdita/config/condAttrList.csv`

   사용 중인 버전에 따라 `libs/fmdita/config/condAttrList.xml` 파일도 같은 위치에서 사용할 수 있습니다.
...
Experience Manager Guides 설정을 기반으로 작성 템플릿을 구성하기 위한 지침은 Cloud Service 또는 온프레미스를 참조하십시오.
...
AEM Guides에서는 작성 템플릿을 손쉽게 만들 수 있습니다. 시스템 관리자는 편집기를 사용하여 제작 템플릿을 처음부터 만들 수 있습니다. 그런 다음 글로벌 프로필에 새 템플릿을 추가하거나 폴더별 프로필을 사용하여 특정 폴더에 할당할 수 있습니다.
...
1. **만들기**&#x200B;를 클릭합니다.
   ...
   편집기에서 편집할 템플릿을 열거나 템플릿 저장소 위치에 템플릿 파일을 저장하도록 선택할 수 있습니다. 템플릿이 생성되면 작성자의 요구에 따라 편집기를 사용하여 템플릿을 사용자 정의할 수 있습니다. 템플릿이 준비되면 전역 또는 폴더 수준 프로필과 연결해야 합니다.
...
글로벌 또는 엔터프라이즈 수준에서 구성된 사전 설정 외에도 게시자는 여전히 더 많은 출력 사전 설정을 만들 수 있는 권한을 갖습니다. 그러나 이러한 사전 설정은 해당 사전 설정이 생성되는 DITA 맵에 연결되어 있습니다. 사용 중인 설정에 따라 DITA 맵에 대한 일반 출력 사전 설정을 만드는 방법에 대한 자세한 내용은 Adobe Experience Manager Guides 사용에서 *출력 사전 설정 만들기, 편집, 복제 또는 제거*&#x200B;를 참조하십시오.
...
1. 출력 사전 설정을 만들거나 편집하려면 다음 중 하나를 수행하십시오.
   ...
   - 선택한 사전 설정의 편집 구성을 열려면 **편집**&#x200B;을 클릭하세요.
     ...
     사용 중인 설정에 따른 출력 사전 설정 설정에 대한 자세한 내용은 Adobe Experience Manager Guides 사용의 *출력 사전 설정 이해*&#x200B;를 참조하십시오.
...
## 스마트 도움말 및 작성을 위한 AI Assistant 구성(Cloud Service 전용)
...
AI 기반의 스마트 제안을 구성하고 작성자가 기존 콘텐츠를 재사용하고 올바르고 일관된 콘텐츠 참조를 쉽게 만들 수 있도록 지원할 수 있습니다. **AI 구성** 탭을 사용하면 편집기의 AI 도우미 패널에서 **재사용 가능한 콘텐츠 제안**&#x200B;의 설정을 제어할 수 있습니다.
...
1. 프로필 페이지에서 **AI 구성** 탭을 선택합니다.
   ...
   글로벌 프로필의 ![AI 구성 탭](assets/global-profile-AI-configuration-cs.png) {width="800"}

1. **편집**&#x200B;을 선택합니다.
1. 관리자는 다음 설정을 구성할 수 있습니다.

   **최소 문자**: 작성자가 제안 사항을 가져오기 위해 선택해야 하는 최소 문자 수를 입력합니다. 예를 들어 이 숫자가 40이면 작성자는 최소 40자를 선택하여 스마트 제안을 확인해야 합니다.

   최소 문자 요구 사항을 충족하지 않는 선택 항목의 경우 다음 메시지가 AI 지원 패널에 표시됩니다.

   ![](assets/smart-suggestions-character-limit.png)

   하지만 제안 사항이 없는 일반 선택 항목의 경우 다음 메시지가 표시됩니다.

   ![](assets/smart-suggestions-select-another-text-message.png)

   따라서 작성자는 부족한 문자 선택으로 인해 제안을 사용할 수 없는지 또는 일치하는 콘텐츠가 없는지 확인하는 데 도움이 됩니다.

   **최대 제안 수**: 콘텐츠를 작성하는 동안 작성자가 얻을 수 있는 최대 제안 수를 입력합니다. 예를 들어 이 숫자가 5이면 작성자는 5개 이하의 스마트 제안을 볼 수 있습니다.

   **파일 및 폴더**: 스마트 제안을 표시할 폴더를 선택하십시오. 폴더 프로필에서 지정된 폴더 경로의 하위 폴더만 선택할 수 있습니다. 자세한 내용은 [폴더 프로필 제한](#folder-profile-restrictions)을 참조하세요.

   *컨텐츠의 일관성을 유지하려면 목록에 있는 두 항목 사이에 공통 파일이 없는 것이 좋습니다*. 파일과 폴더를 선택하면 해당 파일과 폴더가 나열됩니다.

1. **저장**&#x200B;을 클릭합니다.

   >[!NOTE]
   >
   > 파일을 저장하면 폴더 프로필의 마지막 색인 상태가 맨 위에 표시됩니다.

편집기에서 작성하는 동안 콘텐츠 참조를 추가하기 위해 [AI 기반 스마트 제안](../user-guide/authoring-ai-based-smart-suggestions.md)을 보고 추가하는 방법에 대해 자세히 알아보세요.

### 폴더 프로필 제한 사항

스마트 제안이 효과적으로 작동하도록 하려면 폴더를 색인화할 때 다음 사항을 염두에 두십시오.

1. AI 어시스턴트가 작성자에게 스마트 제안을 제공하려면 폴더 프로필을 통해 콘텐츠를 인덱싱해야 합니다.
2. 색인화를 위해 폴더를 지정할 때 현재 폴더 프로필 아래에 있는 폴더만 추가할 수 있습니다. 이 폴더 프로필 외부에 폴더를 추가하려고 하면 경고가 트리거됩니다.

   ![](assets/warning-message-indexing.png)

   이 제한 사항은 폴더 수준 프로필에만 적용됩니다. 전역 프로필은 경로 제한을 적용하지 않으며 다른 폴더 프로필에 없는 폴더를 색인화할 수 있습니다.
3. 인덱싱을 위해 상위 폴더를 추가하면 이미 나열된 하위 폴더가 자동으로 제거되어 중복을 방지할 수 있습니다. 이미 인덱싱된 부모의 하위 폴더를 추가하면 경고도 트리거됩니다.

   ![](assets/parent-child-warning-message-indexing.png)
4. 인덱싱된 폴더 내의 파일을 업데이트, 이동 또는 삭제하면 자동 리인덱싱이나 인덱스에서 제거됩니다.
5. 각 색인화 시도에 대해 다음과 같은 색인화 상태가 표시됩니다.

   - 진행 중: 색인화가 진행 중임을 나타냅니다.
   - 색인화 완료: 색인화가 성공적으로 완료되었음을 나타냅니다.
   - 색인화 실패: 색인화가 실패했음을 나타냅니다.
   - 동기화되지 않음: 현재 색인화 상태를 확인할 수 없을 때 일반적으로 업그레이드 또는 마이그레이션 후 색인화가 동기화되지 않았음을 나타냅니다. 색인화를 다시 시도하여 상태를 새로 고치고 업데이트할 수 있습니다.

   인덱싱이 실패하면 문제를 해결하고 해결하기 위한 **오류 로그 보기** 및 **인덱싱 다시 시도** 옵션이 제공됩니다.
...
6. 각 폴더 프로필에 대해 마지막 인덱스 시간의 타임스탬프가 표시됩니다.
...
Experience Manager Guides 설정을 기반으로 Cloud Service 또는 온프레미스 탭에 대한 정보를 제공합니다.
...
이 설정은 편집기의 도구 모음 및 기타 사용자 인터페이스 요소를 제어합니다. **다운로드** 아이콘을 선택하여 로컬 시스템에서 `ui\_config.json` 파일을 다운로드합니다. 그런 다음 파일을 변경하고 동일한 파일을 업로드할 수 있습니다. 에서 파일을 업로드하는 위치, 글로벌 또는 폴더 수준 프로필에 따라 변경 사항이 적절하게 적용됩니다. `ui\_config.json file`을(를) 사용하여 XML 편집기를 사용자 지정하는 방법에 대한 자세한 내용은 [도구 모음 사용자 지정](customize-toolbar.md#)을 참조하십시오.
...
이 설정은 편집기의 도구 모음 및 기타 사용자 인터페이스 요소를 제어합니다. **다운로드** 아이콘을 클릭하여 로컬 시스템에서 최신 ui\_config.json 파일을 다운로드합니다. 그런 다음 파일을 변경하고 동일한 파일을 업로드할 수 있습니다. **기본 다운로드**&#x200B;아이콘을 클릭하여 로컬 시스템에 기본 ui\_config.json 파일을 다운로드합니다. 언제든지 기본 파일을 다운로드하여 변경하고 업로드할 수 있습니다.에서 파일을 업로드하는 위치, 글로벌 또는 폴더 수준 프로필에 따라 변경 사항이 적절하게 적용됩니다. ui\_config.json 파일을 사용하여 XML 편집기를 사용자 지정하는 방법에 대한 자세한 내용은 [도구 모음 사용자 지정](customize-toolbar.md#)을 참조하십시오.

>[!ENDTABS]

**CSS 템플릿 레이아웃**

이 섹션에서 사용할 수 있는 파일을 다운로드하여 편집기에서 미리 보거나 편집할 수 있도록 열 때 문서의 디자인을 사용자 지정합니다. 다운로드할 수 있는 기본 CSS 파일은 테스트 파일일 뿐이며, 이는 사용자 지정에 사용하면 안 됩니다. 편집기의 사용자 지정 내용이 포함된 CSS 파일을 만들고 업로드할 수 있습니다. 예를 들어 다음 코드를 사용하여 .css 파일을 만들 수 있습니다.

```
.title {    font-size: 9em;}
```

이 파일을 저장하고 CSS 템플릿 레이아웃 섹션에 업로드합니다. 다음에 파일을 다운로드하면 편집기에서 사용 중인 최신 CSS 파일을 가져옵니다.
...
>[!IMPORTANT]
>
> 폴더 수준 구성을 적용하려면 사용자는 편집기의 사용자 환경 설정에서 프로필을 선택해야 합니다.
