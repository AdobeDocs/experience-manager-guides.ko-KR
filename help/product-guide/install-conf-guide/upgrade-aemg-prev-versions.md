...
2. 업그레이드 스크립트 패키지 다운로드: Adobe 소프트웨어 배포 포털(다운로드: `.zip`)에서 **&quot;XML Documentation 솔루션 4.0 업그레이드 패키지&quot;**&#x200B;를 검색합니다.
...
| 예상 응답 | - 필요한 모든 노드를 이동할 수 있는 경우 합격 검사를 받게 됩니다. <br>- 대상 위치에 노드가 있으면 관련 오류가 발생합니다. 저장소 \(노드 /var/dxml\ 삭제)를 정리하고 업그레이드 패키지를 다시 설치한 다음 이 끝점을 다시 트리거합니다. <br>**참고:** 대상 위치가 3.x Experience Manager Guides에서 이전에 사용되지 않으므로 일반적인 오류가 아닙니다. <br> - 이 스크립트가 성공하지 못하면 계속 진행하여 고객 성공 팀에 보고하지 마십시오. |
...
| 예상 응답 | - 마이그레이션이 성공하면 XML Documentation 솔루션 버전 4.0.<br>- 오류가 있는 경우 마지막 체크포인트로 복원하고 API 출력과 함께 오류 로그를 고객 성공 팀과 공유할 수 있습니다. |
...
2. 모든 번역 작업을 마감했습니다.
3. `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`에 대해 로그 수준을 `INFO`(으)로 설정하고 새 로그 파일(예: `logs/translation_upgrade.log`)로 로그인합니다.
...
1. [Adobe 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)에서 **4.2** 패키지를 다운로드합니다.
2. 4.2 패키지를 설치합니다.
...
>[!TIP]
>
> **4.2.1** 버전 위에 **핫픽스4.2.1.3**&#x200B;을(를) 설치하는 것이 좋습니다.
...
2. 모든 번역 작업을 닫습니다.
3. `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`에 대해 로그 수준을 `INFO`(으)로 설정하고 새 파일(예: `logs/translation_upgrade.log`)로 로그인합니다.
...
1. [Adobe 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)에서 **4.2.1** 패키지를 다운로드합니다.
2. 4.2.1 패키지를 설치합니다.
...
**4.3.1** 버전을 사용하는 경우 **4.3.1.5**(으)로 바로 업그레이드할 수 있습니다.

### 버전 4.3.1.5 설치

1. [Adobe 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)에서 **4.3.1.5** 패키지를 다운로드합니다.
...
**4.3.1**, **4.3.0**, **4.2** 또는 **4.2.1(핫픽스 4.2.1.3)**&#x200B;을(를) 사용하는 경우 **4.4.0**(으)로 바로 업그레이드할 수 있습니다.
...
1. **DAM 자산 업데이트 워크플로우**&#x200B;에 대한 &quot;*노드가 생성됨*&quot;의 런처- `"jcr:content/jcr:mimeType!=video"` 조건의 경우 &#39;Globbing&#39; 값은 다음과 같아야 합니다.
   ...
   - 조건 &quot;`jcr:content/jcr:mimeType!=video`&quot;에 대한 **DAM 자산 업데이트 워크플로 -**&#x200B;의 *노드가 수정됨*&#x200B;에 대한 런처, &#39;Globbing&#39; 값은 다음과 같아야 합니다.
...
1. `damAssetLucene` oak 인덱스에서 `/oak:index/damAssetLucene/indexRules/dam:Asset`에 부울 속성 `indexNodeName=true`을(를) 추가합니다.
2. `/oak:index/damAssetLucene/indexRules/dam:Asset/properties` 아래에 이름이 `excerpt`인 새 노드를 추가하고 소스에 표시된 대로 속성을 설정합니다.
   - `name=rep:excerpt`
   - `propertyIndex=true`
   - `notNullCheckEnabled=true`
3. `reindex=true`을(를) 설정하여 `damAssetLucene`을(를) 다시 인덱싱하고 `false`이(가) 될 때까지 기다립니다(몇 시간 정도 걸릴 수 있음).
...
1. (선택 사항) 큰 저장소에 대해 Oak queryLimitReads를 늘립니다
   ...
   **100,000개 이상의 DITA 파일**&#x200B;이 있는 경우 `org.apache.jackrabbit.oak.query.QueryEngineSettingsService`의 `queryLimitReads`을(를) 자산 수보다 큰 값으로 업데이트하고(예: `200000`) 다시 배포한 다음 계속하십시오.
...
- Guides는 `order=50`을(를) 사용하므로 `order` 값 **50**&#x200B;보다 큰 값을 사용하십시오.
