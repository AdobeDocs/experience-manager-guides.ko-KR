...
# 온프레미스를 위한 성능 최적화 권장 사항 {#id213BD0JG0XA}

## 데이터 저장소 구성 \(필수\)

**변경 내용**
`minRecordLength` 속성을 `100` 구성 아래의 `org.apache.jackrabbit.oak.plugins.blob.datastore.FileDataStore.` 값으로 설정합니다. 파일 날짜 저장소 및 S3 데이터 저장소에 대한 자세한 내용은 [AEM 6](https://helpx.adobe.com/kr/experience-manager/6-5/sites/deploying/using/data-store-config.html)에서 노드 저장소 및 데이터 저장소 구성 문서를 참조하십시오.
...
**변경 내용**
`/var/dxml from oak:index/lucene` 제외

>[!NOTE]
>
> AEM Guides은 Lucene 색인을 사용하여 `/var/dxml` 노드에서 콘텐츠를 검색하지 않습니다.

**구성할 때**
콘텐츠를 마이그레이션하기 전에 새 시스템에서 이 변경 작업을 수행하는 경우 `oak:index/lucene`만 업데이트하면 됩니다. 그렇지 않으면 콘텐츠가 이미 마이그레이션된 기존 시스템에서 `oak:index/lucene,`을(를) 변경한 후 Lucene \(*완료하는 데 몇 시간이 걸릴 수 있음*\)에 대한 인덱스를 다시 빌드합니다.
