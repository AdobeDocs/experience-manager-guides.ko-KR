...
- **맞춤법 검사**
  ...
  AEM 맞춤법 검사와 브라우저 맞춤법 검사, 이렇게 두 가지 옵션이 있습니다.

   - **브라우저 맞춤법 검사**: 기본적으로 편집기는 브라우저 맞춤법 검사 기능을 사용합니다. 여기서 맞춤법 검사는 브라우저의 내장된 사전을 사용하여 수행됩니다.

   - **AEM 맞춤법 검사**: AEM 맞춤법 검사로 전환하여 Adobe Experience Manager 사전을 사용할 수 있습니다. 사용자 지정 단어 목록을 추가하도록 사용자 지정할 수도 있습니다. AEM 사전 사용자 지정에 대한 자세한 내용은 [AEM의 기본 사전 사용자 지정](../install-conf-guide/customize-aem-default-dictionary.md)을 참조하십시오.

     >[!NOTE]
     >
     > AEM 맞춤법 검사는 파일의 언어 속성에서 선택한 언어를 사용합니다. 파일에 언어가 지정되지 않은 경우 맞춤법 검사는 AEM 서버 언어로 대체됩니다.
     >맞춤법 검사 결과를 정확하게 확인하려면 AEM에서 선택한 언어에 대한 사전을 사용할 수 있어야 합니다. 구성된 언어에 필요한 사전을 사용할 수 없는 경우 맞춤법 검사 기본값은 `en_us`입니다.
