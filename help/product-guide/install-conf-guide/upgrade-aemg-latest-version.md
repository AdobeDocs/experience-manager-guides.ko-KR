...
현재 버전의 Experience Manager Guides을 버전 5.2.0으로 업그레이드할 수 있습니다.

- 버전 5.0.0, 5.0.3, 5.1.0 또는 5.1.3을 사용 중인 경우 버전 5.2.0으로 바로 업그레이드할 수 있습니다.
...
- [버전 5.2.0으로 업그레이드](#upgrade-to-version-510)
...
>[!NOTE]
>
>5.2.0으로 업그레이드하는 경우 Experience Manager Guides 버전 5.0.0, 5.0.3, 5.1.0 또는 5.1.3에 있어야 합니다. 5.2.0 릴리스의 업그레이드 프로세스는 5.1.0 릴리스와 동일한 단계를 따릅니다.

Experience Manager Guides 5.2.0 업그레이드 프로세스를 시작하기 전에 다음을 확인하십시오.

1. Experience Manager Guides 버전 5.0.0, 5.0.3, 5.1.0 또는 5.1.3으로 업그레이드되었습니다.
...
[Adobe 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)에서 5.2.0 버전 패키지를 다운로드하고 [설치 및 설치 후 업그레이드 워크플로](#installation-and-post-installation-upgrade-workflow)에 제공된 지침에 따라 업그레이드 프로세스를 완료합니다.
...
- API가 `jobId`을(를) 반환합니다. 작업 상태를 확인하려면 작업 ID가 있는 GET 요청을 동일한 끝점인 `http://<server:port\>/bin/guides/map-find/indexing?jobId=\{jobId\}`\(예: ` http://localhost:8080/bin/guides/map-find/indexing?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678`)에 보낼 수 있습니다.
...
>[!NOTE]
>
> 설명서를 따르는 동안 저장 작업을 통해 두 속성(`/oak:index/damAssetLucene`에 대한 `reindex=true` 및 `reindex-async=true`)이 동시에 업데이트되었는지 확인하십시오.
