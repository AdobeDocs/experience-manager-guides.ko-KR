...
변환 프로세스를 실행하려면 [변환을 위해 InDesign 파일을 준비](appendix.md#id195DBF0045Z) 및 [InDesign에서 DITA로의 마이그레이션을 위해 매핑 파일을 준비](appendix.md#id194AF0003HT)해야 합니다. 그런 다음 변환 프로세스를 실행하는 지정된 절차를 따라야 합니다.
...
>[!NOTE]
>
> `stmap.adobeidml.xml` 파일의 구조와 사용자 지정 방법에 대한 자세한 내용은 [부록](appendix.md#id194AF0003HT)의 *InDesign에서 DITA로의 마이그레이션 매핑 파일 준비* 섹션을 참조하십시오.
...
변환 후 구조화된 콘텐츠를 AEM Guides으로 마이그레이션할 수 있습니다.  자세한 내용은 [기존 DITA 콘텐츠 업로드](./migrate-content-upload-existing-dita-content.md)를 참조하십시오.
