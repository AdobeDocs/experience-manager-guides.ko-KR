...
현재 버전의 Experience Manager Guides을 버전 5.1.0 서비스 팩 4로 업그레이드할 수 있습니다.

- 버전 5.1.0 또는 5.1.x 를 사용 중인 경우 버전 5.1.0 서비스 팩 4로 바로 업그레이드할 수 있습니다.
...
다시 `false`이(가) 될 때까지 기다립니다(다시 인덱싱이 완료되었음을 나타냄). 색인의 크기에 따라 몇 시간 정도 소요될 수 있습니다.
...
- API는 jobId를 반환합니다. 작업 상태를 확인하려면 작업 ID가 있는 GET 요청을 동일한 끝점인 `http://<server:port\>/bin/guides/map-find/indexing?jobId=\{jobId\}`\(예: `http://localhost:8080/bin/guides/map-find/indexing?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42`\)에 보낼 수 있습니다.
...
- API는 jobId를 반환합니다. 작업 상태를 확인하려면 작업 ID가 있는 GET 요청을 동일한 끝점인 `http://<server:port\>/bin/guides/map-find/indexing?jobId=\{jobId\}`\(예: ` http://localhost:8080/bin/guides/map-find/indexing?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678`)에 보낼 수 있습니다.
...
>[!TIP]
>
> 버전 5.0.0 서비스 팩 4로 업그레이드하는 방법은 현재 버전의 Experience Manager Guides에 따라 다릅니다. 버전 5.0.x를 사용 중인 경우 버전 5.0.0 서비스 팩 4로 바로 업그레이드할 수 있습니다.
...
>[!NOTE]
>
>5.1.0 서비스 팩 4로 업그레이드하는 경우 Experience Manager Guides 버전 5.1.0 또는 5.1.x에 있어야 합니다.
