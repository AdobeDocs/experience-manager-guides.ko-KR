---
title: Adobe Experience Manager 안내서 업그레이드
description: Adobe Experience Manager Guides를 업그레이드하는 방법에 대해 알아보십시오.
...
현재 버전의 Experience Manager Guides을 버전 5.0.0으로 업그레이드할 수 있습니다.


- 버전 4.6.x, 4.6 또는 4.4를 사용하는 경우 버전 5.0.0으로 직접 업그레이드할 수 있습니다.
...
이 API는 현재 시스템 상태를 평가하고 업그레이드 가능 여부를 보고하도록 설계되었습니다. 이 스크립트를 실행하려면 아래 지정된 엔드포인트를 트리거합니다.
...
이 API는 마이그레이션 매핑&#x200B;**섹션에 설명된**&#x200B;대로 시스템 데이터를 마이그레이션하도록 설계되었습니다.

1. 업그레이드 호환성 확인 API가 실패할 경우 이 스크립트를 실행하지 마십시오(진행하지 않음\).
...
**마이그레이션 매핑**: 위의 API는 원본 위치 아래의 모든 데이터를 타겟 위치로 마이그레이션합니다.
...
1. [Adobe 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)에서 4.0 버전 패키지 다운로드:
   ...
   - 비 UUID 버전의 소프트웨어를 사용하는 경우 &quot;AEM 6.5용 XML 설명서 솔루션용 4.0 비 UUID 릴리스&quot;를 검색하십시오.
...
1. 버전 4.2와 함께 릴리스된 Oxygen 커넥터 플러그인 업그레이드 \(필요한 경우\).
1. 패키지를 설치한 후 브라우저 캐시 지우기.
1. 다음 섹션에 설명된 대로 사용자 지정을 계속 업그레이드합니다.

## 버전 4.2 설치 후 {#id2326F02004K}

>[!IMPORTANT]
>
> 업그레이드된 서버에 하이테크 템플릿 이 표시되지 않습니다. 서버에 하이테크 템플릿을 포함하려면 다음을 복사할 수 있습니다. 소스: /libs/fmdita/pdf/Hi-Tech 대상: /컨텐츠/dam/dita-templates/pdf

Experience Manager Guides를 설치한 후 새로 설치된 버전에서 설정에 적용할 수 있는 다양한 구성을 병합할 수 있습니다.

>[!NOTE]
>
> dam-update-자산 모델은 사용자 정의할 수 있습니다. 따라서 사용자 지정이 수행된 경우 사용자 지정 및 Experience Manager Guides를 모델의 작업 복사본에 동기화 해야 합니다.

1. **DAM 자산 업데이트 작업 과정 \(Post-processing changes\):**
...
1. 구성 요소&#x200B;**삽입 \(프로세스의 마지막 단계로 Experience Manager 가이드 게시물 처리 담당\)을 클릭합니다**.
...
1. **DAM 자산 업데이트 작업 과정** 유효성이 검사되면 해당 런처 구성을 확인하십시오. 이렇게 하려면 AEM 워크플로우 인터페이스로 이동하여 런처를 엽니다.
...
1. **DAM 자산 업데이트 워크플로우**&#x200B;에 대한 &quot;*노드가 생성됨*&quot;의 런처- `"jcr:content/jcr:mimeType!=video"` 조건의 경우 &#39;Globbing&#39; 값은 다음과 같아야 합니다.
   ...
   - 조건 &quot;`jcr:content/jcr:mimeType!=video`&quot;에 대한 **DAM 자산 업데이트 워크플로우 -**&#x200B;의 &quot;*노드가 수정됨*&quot;에 대한 런처,
...
이 문제는 업그레이드에 사용된 쿼리에 대해 색인이 제대로 설정되지 않았기 때문에 발생할 수 있습니다. 다음 해결 방법을 시도해 볼 수 있습니다.
...
>[!TIP]
>
>버전 4.2.1 맨 위에 핫픽스를 4.2.1.3 설치하는 것이 좋습니다.

버전 4.2.1로의 업그레이드는 현재 버전의 Experience Manager Guides에 따라 다릅니다. 버전 4.1, 4.1.x 또는 4.2를 사용하는 경우 버전 4.2.1로 직접 업그레이드할 수 있습니다.
...
1. 모든 번역 작업을 마감했습니다.
1. 클래스에 대한 `com.adobe.fmdita.translationservices.TranslationMapUpgradeScript` 로그 수준을 INFO **로**&#x200B;변경하고 이러한 로그를 새 로그 파일(예:`logs/translation_upgrade.log.`

>[!NOTE]
>
> 모든 활성 검토를 닫아야 합니다. 4.2로 업그레이드하는 동안 검토 작업이 닫히지 않은 경우 진행 중인 이전 검토 작업은 사용자를 이전 검토 페이지로 계속 이동시키고 업그레이드 후 생성된 검토 작업에는 기능의 최신 업데이트가 표시됩니다.

## 버전 4.2.1 설치

1. 4.2.1 버전 패키지는 Adobe Systems Software Distribution Portal에서 [다운로드할 수 있습니다](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html).
...
1. **DAM 자산 업데이트 워크플로우**&#x200B;에 대한 &quot;*노드가 생성됨*&quot;의 런처- `"jcr:content/jcr:mimeType!=video"` 조건의 경우 &#39;Globbing&#39; 값은 다음과 같아야 합니다.
   ...
   - &#39;excludeList&#39;에는 `"event-user-data:changedByWorkflowProcess"`이(가) 있어야 합니다.
   - 조건 &quot;`jcr:content/jcr:mimeType!=video`&quot;의 **DAM 자산 업데이트 워크플로우 -**&#x200B;에 대한 &quot;*노드가 수정됨*&quot;의 런처, &#39;Globbing&#39; 값은 다음과 같아야 합니다.

   ```json
   /content/dam(/((?!/subassets|/translation_output).)*/)renditions/original
   ```

   - `excludeList` 가 있어야 합니다 `"event-user-data:changedByWorkflowProcess"`.

1. 업그레이드가 완료되면 사용자 지정/오버레이의 유효성이 검사되고 새 애플리케이션 코드와 일치하도록 업데이트되었는지 확인합니다. 아래에 몇 가지 예가 나와 있습니다.
   - /libs/fmditaor/libs에서 오버레이된 모든 구성 요소는 새 제품 코드와 비교되어야 하며 업데이트는 오버레이된 파일에서 수행해야 합니다./apps.
   - 제품에서 사용된 모든 clientlib 카테고리의 변경 사항을 검토해야 합니다. 재정의된 구성 \(아래 예\)은 최신 기능을 얻기 위해 최신 구성과 비교해야 합니다.
   - elementmapping.xml
   - ui\_config.json\(폴더 프로필에 설정되었을 수 있음\)
   - 개정 `com.adobe.fmdita.config.ConfigManager`
   - 사용자 지정 코드가 이전 경로를 사용하고 있는지 확인하십시오 \(마이그레이션 매핑[&#128279;](#id2244LE040XA) 섹션에서 언급한 대로\) - 사용자 지정도 예상대로 작동하도록 새 경로로 업데이트해야 합니다.
...
1. 패키지를 설치한 후 브라우저 캐시를 지웁니다.
1. 폴더 프로필의 **XML 편집기 구성** 탭에서 `ui_config.json` 파일을 업그레이드합니다.
...
## 끊어진 링크 보고서를 사용하기 위해 기존 컨텐츠를 게시물 처리하는 단계


기존 컨텐츠를 게시물 처리하고 새로운 끊어진 링크 보고서를 사용하려면 다음 단계를 수행하십시오.

1. (선택 사항) 시스템에 100,000개 이상의 dita 파일이 있는 경우 under `org.apache.jackrabbit.oak.query.QueryEngineSettingsService` 를 `queryLimitReads` 더 큰 값(예: 200,000)으로 업데이트한 다음 다시 배포합니다.
...
1. 다음 API를 실행하여 모든 파일에서 후처리를 실행합니다.
   ...
   |---|---|
   | 요청 유형 | **GET** |
   | 매개 변수 | jobId: 이전 게시물 요청에서 받은 jobId를 전달합니다. |
   | 예상 응답 | - 작업이 완료되면 GET 요청 이 성공적으로 응답합니다. <br> - 오류가 있는 경우 API 출력과 함께 오류 로그를 고객 성공 팀 팀과 공유합니다.  <br>샘플 URL: `http://<server:port>/bin/guides/reports/upgrade?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678` |
...
1. [Adobe 소프트웨어 배포 포털](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)에서 4.3.1.5 버전 패키지를 다운로드합니다.
...
1. 다음 API를 실행하여 모든 파일에서 후처리를 실행합니다.
   ...
   | 요청 유형 | **GET** |
   | 매개 변수 | jobId: 이전 post 요청에서 받은 jobId를 전달합니다. |
   | 예상 응답 | - 작업이 완료되면 GET 요청이 성공으로 응답합니다. <br> - 오류가 있는 경우 API 출력과 함께 오류 로그를 고객 성공 팀 팀과 공유합니다.  <br>샘플 URL: `http://<server:port>/bin/guides/reports/upgrade?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678` |

1. 1단계에서 변경한 경우 기본값 또는 이전의 기존 값으로 `queryLimitReads` 되돌립니다.
...
1. 패키지 설치를 완료한 후 로그에서 다음 메시지를 기다립니다.
   ...
   - 번역 맵을 포팅하는 동안 예외 발생
   - v1에서 v2로의 번역 맵을 속성에 대해 포트할 수 없음
1. 버전 4.6.0으로 릴리스된 Oxygen 커넥터 플러그인 업그레이드 \(필요한 경우\).
1. 패키지를 설치한 후 브라우저 캐시 지우기.
...
1. 변경 내용을 완료한 후 오른쪽 상단의 **동기화**&#x200B;를 클릭합니다. 성공 알림을 받게 됩니다.
   ...
   >[!NOTE]
   >
   > 새로 고침 및 사용자 지정된 변경 사항 및 Experience Manager Guides 게시물 처리 단계가 최종 작업 과정 모델에 있는지 확인합니다.
...
1. **DAM 자산 업데이트 작업 과정** 유효성이 검사되면 해당 런처 구성을 확인하십시오. 이렇게 하려면 AEM 워크플로우 인터페이스로 이동하여 런처를 엽니다.
   ...
   DAM 자산 업데이트 작업 과정&#x200B;**에 해당하는 다음 두 런처\(활성 상태여야 함\)를**&#x200B;찾아서 \(필요한 경우\) 변경합니다.
...
- 작업이 완료되면 위의 GET 요청 은 성공으로 응답하고 맵이 실패했는지 여부를 언급합니다. 성공적으로 인덱싱된 맵은 서버 로그에서 확인할 수 있습니다.


>[!NOTE]
>
> 사용자 지정 스키마를 사용하는 경우 카탈로그 통합 옵션의 AEM 저장소 **에서 사용자 지정 DTD 및 XSD catalog.xml 파일의 경로를 정의해야 합니다** .
...
>[!TIP]
>
> 버전 5.0.0으로의 업그레이드는 현재 버전의 Experience Manager Guides에 따라 다릅니다. 버전 4.6.3, 4.6.1, 4.6.0 또는 4.4를 사용하는 경우 버전 5.0.0으로 바로 업그레이드할 수 있습니다.
...
>[!NOTE]
>
> dam-update-asset 모델은 맞춤화될 수 있다. 따라서 사용자 지정이 수행된 경우 사용자 지정 및 Experience Manager Guides를 모델의 작업 복사본에 동기화 해야 합니다.
...
1. 업그레이드가 완료되면 사용자 지정/오버레이가 새 애플리케이션 코드와 일치하도록 확인 및 업데이트되었는지 확인합니다. 다음은 몇 가지 예입니다.
   - /libs/fmditaor/libsfrom에서 오버레이된 모든 구성 요소는 새 제품 코드와 비교해야 하며, 업데이트는 / 앱에서 오버레이된 파일에서 수행해야 합니다.
   - 제품에서 사용된 모든 clientlib 카테고리의 변경 사항을 검토해야 합니다. 최신 기능을 가져오려면 재정의된 모든 구성 \(아래 예제\)을 최신 구성과 비교해야 합니다.
   - elementmapping.xml
   - ui\_config.json\(폴더 프로필에 설정되었을 수 있음\)
   - 개정 `com.adobe.fmdita.config.ConfigManager`

1. damAssetLucene에서 사용자 지정을 추가한 경우 다시 적용해야 할 수 있습니다. 이러한 변경을 수행한 후 reindex를 true로 설정합니다. 이렇게 하면 모든 기존 노드가 사용자 지정으로 다시 색인화됩니다. 완료되면 색인 재지정 플래그 가 다시 false로 설정됩니다. 시스템의 자산 수에 따라 몇 시간이 걸릴 수 있습니다.

## Experience Manager Guides 색인을 다시 색인화하는 단계

1. 열고 `crx/de` 색인 경로로 이동합니다.  `/oak:index/guidesAssetProperties`
2. 인덱스 다시 지정 속성을 `true`(기본적으로 `false`)로 설정하고 **모두 저장**&#x200B;을(를) 클릭합니다.
3. 다시 색인이 완료되면 다시 색인 속성이 `false`(으)로 설정되고 색인 수가 1씩 증가합니다.
