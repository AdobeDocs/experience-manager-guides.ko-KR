...
By default, you can view the files by titles. 파일 위로 마우스를 가져가면 파일 제목과 파일 경로를 도구 설명으로 볼 수 있습니다.
...
- **미리 보기**: 파일(.dita, .xml, 오디오, 비디오 또는 이미지)을 열지 않고 빠르게 미리 봅니다. 미리 보기 창의 크기를 조정할 수 있습니다. If the content contains any `<xref>` or `<conref>`, you can select it to open it in a new tab. The title of the file appears in the window. If no title is present, then the filename appears. To close the **Preview** pane, you can either select the close icon or click anywhere outside the pane.
...
- **Add To**: You can choose from the following options:
   - **Favorites**: Adds the selected file to favorites. 기존 또는 새 즐겨찾기 컬렉션에 추가할 수 있습니다.
...
- 개요 보기에서 요소를 클릭하면 작성자 또는 Source 보기에서 요소의 콘텐츠가 선택됩니다.The Outline view remains in sync with the Author and Source view. If you make any changes in any view, you can see them in the Outline view. For example, if you add a paragraph or update an element in the Author view, it is shown in the Outline view.
...
If your administrator has created a profile for attributes, then you&#39;ll get those attributes along with their configured values. You can also assign display  attributes configured by your administrator under the **Display Attributes** tab in the editor settings. 요소에 대해 정의된 속성이 레이아웃 및 아웃라인 뷰에 표시됩니다.
...
1. ![](images/active-review-tasklist-icon.svg) 검토 선택   왼쪽 패널에서 **검토** 패널이 열립니다.  사용자가 속한 모든 검토 프로젝트 및 검토 프로젝트 내의 활성 검토 작업이 표시됩니다.
